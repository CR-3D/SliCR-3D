...
def makeFullSettingDict(gCodeSettingDict: dict) -> dict:
    ...
    AddManualSettingsDict = {
        ...
        "ExtendIntoPerimeter": 1.5 * gCodeSettingDict.get("perimeter_extrusion_width"),
        # min=0.5extrusionwidth!, extends the Area for arc generation, put higher to go through small passages. Unit:mm
        ...
        "MinArea": 10 * 8,  # Unit:mm2
        "MinBridgeLength": 40,  # Unit:mm
        ...
        # if the gcode line is closer than this distance to an infill polygon the cooling settings will be applied. Unit:mm
        ...
        # Startpoint for Arc generation is chosen close to the middle of the StartLineString and at a corner. Higher=>Cornerselection more important.
        "DistanceBetweenPointsOnStartLine": 0.1,  # used for redestribution, if start fails.
        "GCodeArcPtMinDist": 0.2,  # min Distance between points on the Arcs to for seperate GCode Command. Unit:mm
        "ExtendArcDist": 1.0,
        # extend Arcs tangentially for better bonding bewteen them, only end-piece affected(yet), Unit:mm
        "HilbertFillingPercentage": 100,
        # infillpercentage of the massive layers with special cooling. Uses Hilbert Curve, works not quite right yet.
        "HilbertInfillExtrusionMultiplier": 1.05,
        "HilbertTravelEveryNSeconds": 6,
        # when N seconds are driven it will continue printing somewhere else (very rough approx).
        "MinStartArcs": 4,  # how many arcs shall be generated in first step
        "PointsPerCircle": 45,
        # each Arc starts as a discretized circle. Higher will slow down the code but give more accurate results for the arc-endings.
        ...
        # fill the overhang at least XX%, else send a warning. Easier detection of errors in small/delicate areas. Unit:Percent
        "UseLeastAmountOfCenterPoints": False,
        # always generates arcs until rMax is reached, divide the arcs into pieces in needed. reduces the amount of centerpoints.

        # settings for easier debugging:
        "plotStart": False,
        # plot the detected geoemtry in the prev Layer and the StartLine for Arc-Generation, use for debugging
...
def main(gCodeFileStream, path2GCode, skipInput) -> None:
    ...
    gCodeSettingDict = readSettingsFromGCode2dict(gCodeLines, {"Fallback_nozzle_diameter": 0.4,
                                                               "Fallback_filament_diameter": 1.75})  #ADD FALLBACK VALUES HERE
    ...
    if gCodeFileStream:
        ...
        for idl, layerlines in enumerate(layers):
            ...
            layer.addZ(layer.z if layer.z else None)
            layer.addHeight()
        ...
        for idl, layer in enumerate(layerobjs):
            ...
            if idl < 1:
                ...
                if layer.validpolys:
                    ...
                    idoffset = 1
                    currZ = layer.z
                    layer.addZ(layer.z + 0.05 if layer.z else None)
                    ...
                    for poly in layer.validpolys:
                        ...
                        MaxDistanceFromPerimeter = parameters.get(
                            "MaxDistanceFromPerimeter")  # how much 'bumpiness' you accept in the outline. Lower will generate more small arcs to follow the perimeter better (corners!). Good practice: 2 perimeters+ threshold of 2width=minimal exact touching (if rMin satisfied)
                        ...
                        concentricArcs = generateMultipleConcentricArcs(startpt, rMinStart, rMax,
                                                                        boundaryWithOutStartLine, remainingSpace,
                                                                        parameters)
                        ...
                        if len(concentricArcs) < parameters.get("MinStartArcs"):
                            ...
                            if len(concentricArcs) < parameters.get("MinStartArcs"):  #still insuff start: try random
                                ...
                                for idr in range(10):
                                    ...
                                    concentricArcs = generateMultipleConcentricArcs(startpt, rMinStart, rMax,
                                                                                    boundaryWithOutStartLine,
                                                                                    remainingSpace, parameters)
                                    if len(concentricArcs) >= parameters.get("MinStartArcs"):
                                        break
                                if len(concentricArcs) < parameters.get("MinStartArcs"):
                                    for idr in range(10):
                                        startpt = getStartPtOnLS(redistribute_vertices(startLineString, 0.1),
                                                                 parameters, choseRandom=True)
                                        concentricArcs = generateMultipleConcentricArcs(startpt, rMinStart, rMax,
                                                                                        boundaryWithOutStartLine,
                                                                                        remainingSpace, parameters)
                                        if len(concentricArcs) >= parameters.get("MinStartArcs"):
                                ...
                                if len(concentricArcs) < parameters.get("MinStartArcs"):
                                    warnings.warn(
                                        "Initialization Error: no concentric Arc could be generated at startpoints, moving on")
                                    continue
                        ...
                        while idx < len(finalarcs):
                            ...
                            if curArc.poly.geom_type == "MultiPolygon":
                                farthestPointOnArc, longestDistance, NearestPointOnPoly = get_farthest_point(
                                    curArc.poly.geoms[0], poly, remainingSpace)
                            else:
                                farthestPointOnArc, longestDistance, NearestPointOnPoly = get_farthest_point(
                                    curArc.poly, poly, remainingSpace)
                            if not farthestPointOnArc or longestDistance < MaxDistanceFromPerimeter:  #no more pts on arc
                                idx += 1  #go to next arc
                                continue
                            startpt = move_toward_point(farthestPointOnArc, curArc.center,
                                                        parameters.get("ArcCenterOffset", 2))
                            concentricArcs = generateMultipleConcentricArcs(startpt, rMin, rMax, poly.boundary,
                                                                            remainingSpace, parameters)
                            arcBoundarys = getArcBoundarys(concentricArcs)
                            #print(f"number of concentric arcs generated:",len(concentricArcs))
                            ...
                            if parameters.get("plotArcsEachStep"):
                                plt.title(
                                    f"Iteration {idx}, Total No Start Points: {len(finalarcs)}, Total No Arcs: {len(arcs)}")
                            ...
                            if len(finalarcs) == 1 and idx == 1 and remainingSpace.area / poly.area * 100 > 50 and not triedFixing:
                                ...
                                triedFixing = True
                                print(
                                    "the arc-generation got stuck at a thight spot during startup. Used Automated fix:set ArcCenterOffset to 0")
                        ...
                        if remain2FillPercent > 100 - parameters.get("WarnBelowThisFillingPercentage"):
                            warnings.warn(
                                f"layer {idl}: The Overhang Area is only {100 - remain2FillPercent:.0f}% filled with Arcs. Please try again with adapted Parameters: set 'ExtendIntoPerimeter' higher to enlargen small areas. lower the MaxDistanceFromPerimeter to follow the curvature more precise. Set 'ArcCenterOffset' to 0 to reach delicate areas. ")
                        ...
                        arcOverhangGCode.append(
                            f"M106 S{np.round(parameters.get('bridge_fan_speed', 100) * 2.55)}\n")  #turn cooling Fan on at Bridge Setting
    ...
    if gcodeWasModified:
        ...
        print(
            f"Analysed {len(layerobjs)} Layers, but no matching overhangs found->no arcs generated. If unexpected: look if restricting settings like 'minArea' or 'MinBridgeLength' are correct.")
...
def makePolygonFromGCode(lines: list) -> Polygon:
    ...
    for line in lines:
        if ";WIPE" in line:
            break
        if "G1" in line:
            p = Layer().getPtfromCmd(line=line)
            if p:
                pts.append(p)
    if len(pts) > 2:
        return Polygon(pts)
...
class Layer:
    ...
    def makeExternalPerimeter2Polys(self) -> None:
        ...
        for idf, fe in enumerate(self.features):
            ...
            if "External" in ftype or ("Overhang" in ftype and extPerimeterIsStarted) or (
                    "Overhang" in ftype and self.dontPerformPerimeterCheck):  #two different types of perimeter to for a poly: external perimeter and overhang perimeter + option for manual errorhandling, when there is no feature "external"
            ...
            if (idf == len(self.features) - 1 and extPerimeterIsStarted) or (extPerimeterIsStarted and not (
                    "External" in ftype or "Overhang" in ftype)):  #finish the poly if end of featurelist or different feature
                poly = makePolygonFromGCode(linesWithStart)
                if poly:
                    self.extPerimeterPolys.append(poly)
                extPerimeterIsStarted = False

    def getPtfromCmd(self, line: str) -> Point:
        x = None
        y = None
        line = line.split(";")[0]
        cmds = line.split(" ")
        for c in cmds:
            if "X" in c:
                ...
                z = float(c[1:])
                if self.lastP:
                    return Point(self.lastP.x, self.lastP.y, z)
                else:
                    return None
    ...
    def spotFeaturePoints(self, featureName: str, splitAtWipe=False, includeRealStartPt=False,
                          splitAtTravel=False) -> list:
        parts = []
        for idf, fe in enumerate(self.features):
            ftype = fe[0]
            lines = fe[1]
    ...
    def verifyinfillpolys(self, minDistForValidation: float = 0.5) -> None:
        ...
        if len(overhangs) > 0:
            if self.parameters.get("PrintDebugVerification"): print(
                f"Layer {self.layernumber}: {len(overhangs)} Overhangs found")
            allowedSpacePolygon = self.parameters.get("AllowedSpaceForArcs")
            if not allowedSpacePolygon:
                input(
                    f"Layer {self.layernumber}: no allowed space Polygon provided to layer obj, unable to run script. Press Enter.")
            ...
            for idp, poly in enumerate(self.polys):
                if not poly.is_valid:
                    if self.parameters.get("PrintDebugVerification"): print(
                        f"Layer {self.layernumber}: Poly{idp} is (shapely-)invalid")
                    continue
                if (not allowedSpacePolygon.contains(poly)) and self.parameters.get("CheckForAllowedSpace"):
                    if self.parameters.get("PrintDebugVerification"): print(
                        f"Layer {self.layernumber}: Poly{idp} is not in allowedSpacePolygon")
                    continue
                if poly.area < self.parameters.get("MinArea"):
                    if self.parameters.get("PrintDebugVerification"): print(
                        f"Layer {self.layernumber}: Poly{idp} has to little area: {poly.area:.2f}")
                    continue
                for ohp in overhangs:
                    if poly.distance(ohp) < minDistForValidation:
                        if ohp.length > self.parameters.get("MinBridgeLength"):
                            ...
                            break
                if self.parameters.get("PrintDebugVerification"): print(
                    f"Layer {self.layernumber}: Poly{idp} is not close enough to overhang perimeters")
    ...
    def createHilbertCurveInPoly(self, poly: Polygon):
        ...
        mmBetweenTravels = (self.parameters.get("aboveArcsInfillPrintSpeed") / 120) * self.parameters.get(
            "HilbertTravelEveryNSeconds")
        ...
        iterationCount = int(
            np.ceil(np.log((a * l + w) / w) / np.log(2)))  # + applied ceiling function to ensucre full coverage.
        scale = w / a  #l/(2**iterationCount-1)/a
        maxidx = int(2 ** (dimensions * iterationCount) - 1)
        locs = decode(np.arange(maxidx), 2,
                      iterationCount)  # hilbertidx->(x,y) first argument: idx, second: dimensions, third: bits per dim
    ...
    def isClose2Bridging(self, line: str, minDetectionDistance: float = 3):
        ...
        ls = LineString([(p.x, p.y), (self.lastP.x, self.lastP.y)])
        self.lastP = p
        for poly in self.oldpolys:
...
def create_circle(p: Point, radius: float, n: int) -> Polygon:
    x = p.x
    y = p.y
    return Polygon([[radius * np.sin(theta) + x, radius * np.cos(theta) + y] for theta in
                    np.linspace(0, 2 * np.pi - 2 * np.pi / n, int(n))])


def get_farthest_point(arc: Polygon, base_poly: Polygon,
                       remaining_empty_space: Polygon):  #function ported from Steven McCulloch
    """
...
def move_toward_point(start_point: Point, target_point: Point, distance: float, dz: float = 1.0) -> Point:
    """Moves a point a set distance toward another point, including a fixed movement in the Z direction"""
    ...
    if start_point.has_z or target_point.has_z:
        dz = dz  # Fixed movement in the Z direction
        # Move the point in the direction of the target by the set distance and adjust the Z-coordinate
        return Point(start_point.x + dx * distance, start_point.y + dy * distance, start_point.z + dz)
...
def generateMultipleConcentricArcs(startpt: Point, rMin: float, rMax: float, boundaryLineString: LineString,
                                   remainingSpace: Polygon, kwargs={}) -> list:
    arcs = []
    r = rMin
    while r <= rMax:
...
def readSettingsFromGCode2dict(gcodeLines: list, fallbackValuesDict: dict) -> dict:
    ...
    for line in gcodeLines:
        if "; SliCR-3D_config = begin" in line:
            isSetting = True
            continue
        if isSetting:
            ...
            if len(setting) == 2:
                try:
                    gCodeSettingDict[setting[0].strip(" ")] = literal_eval(
                        setting[1])  # automaticly convert into int,float,...
                except:
                    gCodeSettingDict[setting[0].strip(" ")] = setting[
                        1]  # leave the complex settings as strings. They shall be handled individually if necessary
            elif len(setting) > 2:
                gCodeSettingDict[setting[0].strip(" ")] = setting[1:]
                warnings.warn(
                    f"PrusaSlicer Setting {setting[0]} not in the expected key/value format, but added into the settings-dictionarry")
            else:
                print("Could not read setting from PrusaSlicer:", setting)
    if "%" in str(gCodeSettingDict.get(
            "perimeter_extrusion_width")):  #overwrite Percentage width as suggested by 5axes via github
        gCodeSettingDict["perimeter_extrusion_width"] = gCodeSettingDict.get("nozzle_diameter") * (
                    float(gCodeSettingDict.get("perimeter_extrusion_width").strip("%")) / 100)
    isWarned = False
    for key, val in gCodeSettingDict.items():
        if isinstance(val, tuple):
            if gCodeSettingDict.get("Fallback_" + key):
                ...
                if not isWarned:
                    warnings.warn(
                        f"{key} was specified as tuple/list, this is normal for using multiple extruders. For all list values First values will be used. If unhappy: Add manual fallback value by searching for ADD FALLBACK in the code. And add 'Fallback_<key>:<yourValue>' into the dictionary.")
                    isWarned = True
    return gCodeSettingDict
...
def checkforNecesarrySettings(gCodeSettingDict: dict) -> bool:
    ...
    keys_to_check = ["extrusion_width", "perimeter_extrusion_width", "solid_infill_extrusion_width", "bridge_speed"]
    for key in keys_to_check:
        if key in gCodeSettingDict:
            gCodeSettingDict[key] = convert_percent_to_float(gCodeSettingDict.get(key))

    # Check the conditions with the possibly converted values
    if gCodeSettingDict.get("extrusion_width") < 0.001 or gCodeSettingDict.get(
            "perimeter_extrusion_width") < 0.001 or gCodeSettingDict.get("solid_infill_extrusion_width") < 0.001:
        warnings.warn(
            "Script only works with extrusion_width and perimeter_extrusion_width and solid_infill_extrusion_width > 0. Change in PrusaSlicer accordingly.")
        return False

    if not gCodeSettingDict.get("s_overhangs"):
    ...
    if gCodeSettingDict.get("bridge_speed") > 5:
        warnings.warn(
            f"Your Bridging Speed is set to {gCodeSettingDict.get('bridge_speed'):.0f} mm/s in PrusaSlicer. This can cause problems with warping. <= 5 mm/s is recommended")

    if gCodeSettingDict.get("infill_first"):
        warnings.warn(
            "Infill set in PrusaSlicer to be printed before perimeter. This can cause problems with the script.")

    if gCodeSettingDict.get("external_perimeters_first"):
        warnings.warn(
            "PrusaSlicer-Setting: External perimeter is printed before inner perimeters. Change for better overhang performance. ")

    if not gCodeSettingDict.get("avoid_crossing_perimeters"):
        warnings.warn(
            "PrusaSlicer-Setting: Travel Moves may cross the outline and therefore cause artefacts in arc generation.")
    return True
...
def calcEStepsPerMM(settingsdict: dict, layerheight: float = None) -> float:
    if layerheight:  # case: printing on surface.
        ...
        eVol = (settingsdict.get("nozzle_diameter") / 2) ** 2 * np.pi * settingsdict.get("ArcExtrusionMultiplier",
                                                                                         1)  #printing in midair will result in circular shape. Scource: https://manual.slic3r.org/advanced/flow-math
...
def arc2GCode(arcline: LineString, eStepsPerMM: float, arcidx=None, kwargs={}) -> list:
    ...
    arcPrintSpeed = np.clip(arcline.length / (kwargs.get("ArcSlowDownBelowThisDuration", 3)) * 60,
                            kwargs.get("ArcMinPrintSpeed", 1 * 60),
                            kwargs.get('ArcPrintSpeed', 2 * 60))  # *60 bc unit conversion:mm/s=>mm/min
    for idp, p in enumerate(pts):
        if idp == 0:
            p1 = p
        ...
        if idp == len(pts) - 1:
            GCodeLines.append(
                p2GCode(pExtend, E=extDist * eStepsPerMM))  #extend arc tangentially for better bonding between arcs
    ...
    for idc, curvepts in enumerate(allhilbertpts):
        for idp, p in enumerate(curvepts):
            if idp == 0:
                ...
                hilbertGCode.append(
                    p2GCode(p, E=eStepsPerMM * p.distance(lastP), F=parameters.get("aboveArcsInfillPrintSpeed")))
            else:
