...
class Layer():
    ...
    def createHilbertCurveInPoly(self, poly: Polygon):
        ...
        mmBetweenTravels = (self.parameters.get("aboveArcsInfillPrintSpeed") / 60) * self.parameters.get(
...
def readSettingsFromGCode2dict(gcodeLines: list, fallbackValuesDict: dict) -> dict:
    ...
    for line in gcodeLines:
        if "; SuperSlicer_config = begin" in line:
            isSetting = True
            continue
        if isSetting:
...
def checkforNecesarrySettings(gCodeSettingDict: dict) -> bool:
    ...
    if gCodeSettingDict.get("extrusion_width") < 0.001 or gCodeSettingDict.get(
        ...
        warnings.warn(
            "Script only works with extrusion_width and perimeter_extrusion_width and solid_infill_extrusion_width>0. Change in PrusaSlicer acordingly.")
        return False
    ...
    if gCodeSettingDict.get("bridge_speed") > 5:
        warnings.warn(
            f"Your Bridging Speed is set to {gCodeSettingDict.get('bridge_speed'):.0f} mm/s in PrusaSlicer. This can cause problems with warping.<=5mm/s is recommended")
