...
void CalibrationPressureAdvDialog::create_geometry(wxCommandEvent& event_args) {
   ...
   std::unordered_map<std::string, std::string> er_width_ToOptionKey = {
    ...
    std::vector<size_t> objs_idx = plat->load_files(items, LoadStrategy::LoadModel);
...
void CalibrationPressureAdvDialog::create_buttons(wxStdDialogButtonSizer* buttons) {
    ...
    if (prefix != "unsupported firmware type") {
        ...
        generateButton->SetBackgroundColour(background_color);
        generateButton->SetForegroundColour(text_color);
        ...
        closeButton->SetBackgroundColour(background_color);
        closeButton->SetForegroundColour(text_color);
