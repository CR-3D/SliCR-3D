...
void PageDiameters::apply_custom_config(DynamicPrintConfig &config)
{
    ...
    auto set_extrusion_width = [&config, opt_nozzle](const char *key, double dmr) {
        ...
        sprintf(buf, "%.2lf", dmr * opt_nozzle->get_values().front() / 0.4);
...
ConfigWizardIndex::ConfigWizardIndex(wxWindow *parent)
    : wxPanel(parent)
    , bg(ScalableBitmap(parent, "SliCR-3D_192px_transparent.png", 192))
...
const std::string& Materials::get_filament_type(const Preset *preset)
{
    ...
    if (opt != nullptr && opt->get_values().size() > 0) {
        return opt->get_values()[0];
    } else {
        return UNKNOWN;
    }
}
