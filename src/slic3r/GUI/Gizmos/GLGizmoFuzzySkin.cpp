...
void GLGizmoFuzzySkin::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    auto draw_text_with_caption = [&caption_max, this](const std::string &caption, const std::string &text) {
        m_imgui->text_colored(ImGuiWrapper::COL_TURQUOISE_LIGHT, caption);
    ...
    if (ImGui::IsItemHovered())
        m_imgui->tooltip(_u8L("Paints facets according to the chosen painting brush."), max_tooltip_width);
    ...
    if (ImGui::IsItemHovered())
        m_imgui->tooltip(_u8L("Paints neighboring facets whose relative angle is less or equal to set angle."), max_tooltip_width);
    ...
    if (m_tool_type == ToolType::BRUSH) {
        ...
        if (ImGui::IsItemHovered())
            m_imgui->tooltip(_u8L("Paints all facets inside, regardless of their orientation."), max_tooltip_width);
        ...
        if (ImGui::IsItemHovered())
            m_imgui->tooltip(_u8L("Ignores facets facing away from the camera."), max_tooltip_width);
        ...
        if (ImGui::IsItemHovered())
            m_imgui->tooltip(_u8L("Paints only one facet."), max_tooltip_width);
        ...
        if (ImGui::IsItemHovered())
            m_imgui->tooltip(_u8L("Splits bigger facets into smaller ones while the object is painted."), max_tooltip_width);
