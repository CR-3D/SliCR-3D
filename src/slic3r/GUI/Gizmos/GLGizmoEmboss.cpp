...
bool GLGizmoEmboss::on_is_activable() const {
    const Selection& selection = m_parent.get_selection();
    return on_is_actionable() || (!selection.is_empty() && selection.is_text() );
}
...
void GLGizmoEmboss::trigger_action() {
    if (!m_parent.get_selection().is_single_text() && m_should_trigger_action && !m_parent.get_selection().is_empty())
        create_volume(ModelVolumeType::MODEL_PART);
}
