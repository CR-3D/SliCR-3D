///|/ Copyright (c) Prusa Research 2020 - 2023 David Kocík @kocikdav, Vojtěch Bubník @bubnikv, Oleksandra Iushchenko
///@YuSanka, Filip Sykala @Jony01, Lukáš Matěna @lukasmatena, Enrico Turri @enricoturri1966, Lukáš Hejl @hejllukas,
...
void NotificationManager::PopNotification::count_lines() {
    ...
    if (!m_hypertext.empty()) {
        int prev_end = m_endlines.size() > 1 ? m_endlines[m_endlines.size() - 2] : 0; // m_endlines.size() - 2 because
    we are fitting hypertext instead of last endline if
    (ImGui::CalcTextSize((escape_string_cstyle(text.substr(prev_end, last_end - prev_end)) + m_hypertext).c_str()).x >
    m_window_width - m_window_width_offset) { m_endlines.push_back(last_end); m_lines_count++;
        }
    }
    */
...
void NotificationManager::PopNotification::render_text(ImGuiWrapper &imgui,
    ...
    for (size_t i = 0; i < (m_multiline ? m_endlines.size() : std::min(m_endlines.size(), m_normal_lines_count));
         i++) {
    ...
    if (!m_multiline && m_lines_count > m_normal_lines_count) {
        render_hypertext(imgui, x_offset + ImGui::CalcTextSize((line + " ").c_str()).x,
    ...
    if (!m_text2.empty() && (m_multiline || m_lines_count <= m_normal_lines_count)) {
        ...
        for (size_t i = 0; i < (m_multiline ? m_endlines2.size() : m_normal_lines_count); i++) {
            if (i == 0) // first line X is shifted by hypertext
                ImGui::SetCursorPosX(
                    x_offset + ImGui::CalcTextSize((line + m_hypertext + (line.empty() ? " " : "  ")).c_str()).x);
            else
                ImGui::SetCursorPosX(x_offset);
...
void NotificationManager::PopNotification::render_close_button(ImGuiWrapper &imgui,
                                                               const float win_size_x,
                                                               const float win_size_y,
                                                               const float win_pos_x,
                                                               const float win_pos_y) {
...
void NotificationManager::PopNotification::render_minimize_button(ImGuiWrapper &imgui,
                                                                  const float win_pos_x,
                                                                  const float win_pos_y) {
    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.0f, .0f, .0f, .0f));
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.0f, .0f, .0f, .0f));
...
void NotificationManager::PopNotification::on_more_hypertext_click() { m_multiline = true; }
...
void NotificationManager::ExportFinishedNotification::render_text(ImGuiWrapper &imgui,
                                                                  const float win_size_x,
                                                                  const float win_size_y,
                                                                  const float win_pos_x,
                                                                  const float win_pos_y) {
    ...
    for (size_t i = 0; i < m_lines_count; i++) {
        ...
        if (m_text1.size() >= m_endlines[i]) {
            ...
            if (i == 0 && !m_eject_pending) {
                render_hypertext(imgui, x_offset + ImGui::CalcTextSize(line.c_str()).x + ImGui::CalcTextSize("   ").x,
...
void NotificationManager::ExportFinishedNotification::render_close_button(ImGuiWrapper &imgui,
                                                                          const float win_size_x,
                                                                          const float win_size_y,
                                                                          const float win_pos_x,
                                                                          const float win_pos_y) {
...
void NotificationManager::ExportFinishedNotification::render_eject_button(ImGuiWrapper &imgui,
                                                                          const float win_size_x,
                                                                          const float win_size_y,
                                                                          const float win_pos_x,
                                                                          const float win_pos_y) {
...
void NotificationManager::ProgressBarNotification::render_text(ImGuiWrapper &imgui,
                                                               const float win_size_x,
                                                               const float win_size_y,
                                                               const float win_pos_x,
                                                               const float win_pos_y) {
    ...
    if (m_multiline) {
        ...
        std::string line =
            m_text1.substr(m_endlines[0] + (m_text1[m_endlines[0]] == '\n' || m_text1[m_endlines[0]] == ' ' ? 1 : 0),
...
void NotificationManager::ProgressBarNotification::render_bar(ImGuiWrapper &imgui,
                                                              const float win_size_x,
                                                              const float win_size_y,
                                                              const float win_pos_x,
                                                              const float win_pos_y,
    ...
    ImGui::GetWindowDrawList()->AddLine(lineStart, midPoint,
                                        IM_COL32((int) (orange_color.x * 255), (int) (orange_color.y * 255),
                                                 (int) (orange_color.z * 255), (m_current_fade_opacity * 255.f)),
                                        m_line_height * 0.2f);
    if (render) {
        std::string text;
...
void NotificationManager::ProgressBarNotification::render_bar(ImGuiWrapper &imgui,
                                                              ...
                                                              const float win_pos_x,
                                                              const float win_pos_y) {
    render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y, (m_multiline ? m_line_height / 2 : 0),
               m_percentage, m_render_percentage);
}

//------ProgressBarWithCancelNotification----------------

void NotificationManager::ProgressBarWithCancelNotification::render_close_button(ImGuiWrapper &imgui,
                                                                                 const float win_size_x,
                                                                                 const float win_size_y,
                                                                                 const float win_pos_x,
                                                                                 const float win_pos_y) {
...
void NotificationManager::ProgressBarWithCancelNotification::render_close_button_inner(ImGuiWrapper &imgui,
                                                                                       const float win_size_x,
                                                                                       const float win_size_y,
                                                                                       const float win_pos_x,
                                                                                       const float win_pos_y) {
...
void NotificationManager::ProgressBarWithCancelNotification::render_cancel_button_inner(ImGuiWrapper &imgui,
                                                                                        const float win_size_x,
                                                                                        const float win_size_y,
                                                                                        const float win_pos_x,
                                                                                        const float win_pos_y) {
...
void NotificationManager::ProgressBarWithCancelNotification::render_bar(ImGuiWrapper &imgui,
                                                                        const float win_size_x,
                                                                        const float win_size_y,
                                                                        const float win_pos_x,
                                                                        const float win_pos_y) {
...
void NotificationManager::URLDownloadNotification::render_close_button(ImGuiWrapper &imgui,
                                                                       const float win_size_x,
                                                                       const float win_size_y,
                                                                       const float win_pos_x,
                                                                       const float win_pos_y) {
...
void NotificationManager::URLDownloadNotification::render_close_button_inner(ImGuiWrapper &imgui,
                                                                             const float win_size_x,
                                                                             const float win_size_y,
                                                                             const float win_pos_x,
                                                                             const float win_pos_y) {
...
void NotificationManager::URLDownloadNotification::render_pause_cancel_buttons_inner(ImGuiWrapper &imgui,
                                                                                     const float win_size_x,
                                                                                     const float win_size_y,
                                                                                     const float win_pos_x,
                                                                                     const float win_pos_y) {
...
void NotificationManager::URLDownloadNotification::render_pause_button_inner(ImGuiWrapper &imgui,
                                                                             const float win_size_x,
                                                                             const float win_size_y,
                                                                             const float win_pos_x,
                                                                             const float win_pos_y) {
...
void NotificationManager::URLDownloadNotification::render_open_button_inner(ImGuiWrapper &imgui,
                                                                            const float win_size_x,
                                                                            const float win_size_y,
                                                                            const float win_pos_x,
                                                                            const float win_pos_y) {
...
void NotificationManager::URLDownloadNotification::render_cancel_button_inner(ImGuiWrapper &imgui,
                                                                              const float win_size_x,
                                                                              const float win_size_y,
                                                                              const float win_pos_x,
                                                                              const float win_pos_y) {
...
void NotificationManager::URLDownloadNotification::render_bar(ImGuiWrapper &imgui,
                                                              const float win_size_x,
                                                              const float win_size_y,
                                                              const float win_pos_x,
                                                              const float win_pos_y) {
...
void NotificationManager::PrintHostUploadNotification::count_spaces() {
    ...
    if (m_uj_state == UploadJobState::PB_ERROR || m_uj_state == UploadJobState::PB_COMPLETED_WITH_WARNING) {
        std::string text;
        // text = (m_data.level == NotificationLevel::ErrorNotificationLevel ? ImGui::ErrorMarker :
        // ImGui::WarningMarker); its always progressbar level (not error or warning)
...
void NotificationManager::PrintHostUploadNotification::render_text(ImGuiWrapper &imgui,
                                                                   const float win_size_x,
                                                                   const float win_size_y,
                                                                   const float win_pos_x,
                                                                   const float win_pos_y) {
    // If not completed, the text rendering is very similar to progressbar notification except it doesnt use m_multiline
    // to decide. If completed, whole text is part of m_text_1 and is rendered by PopNotification function.
...
void NotificationManager::PrintHostUploadNotification::render_bar(ImGuiWrapper &imgui,
                                                                  const float win_size_x,
                                                                  const float win_size_y,
                                                                  const float win_pos_x,
                                                                  const float win_pos_y) {
...
void NotificationManager::PrintHostUploadNotification::render_close_button(ImGuiWrapper &imgui,
                                                                           const float win_size_x,
                                                                           const float win_size_y,
                                                                           const float win_pos_x,
                                                                           const float win_pos_y) {
...
void NotificationManager::PrintHostUploadNotification::render_cancel_button(ImGuiWrapper &imgui,
                                                                            const float win_size_x,
                                                                            const float win_size_y,
                                                                            const float win_pos_x,
                                                                            const float win_pos_y) {
    ...
    push_style_color(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f), m_state == EState::FadingOut, m_current_fade_opacity);
    push_style_color(ImGuiCol_TextSelectedBg, ImVec4(0, .75f, .75f, 1.f), m_state == EState::FadingOut,
    m_current_fade_opacity); ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(.0f, .0f, .0f, .0f));
...
bool NotificationManager::SlicingProgressNotification::set_secondary_progress(const std::string &text, float percent) {
    if (m_sp_state ==
        Slic3r::GUI::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_PROGRESS) {
        m_text2 = text;
...
void NotificationManager::SlicingProgressNotification::render_text(ImGuiWrapper &imgui,
                                                                   const float win_size_x,
                                                                   const float win_size_y,
                                                                   const float win_pos_x,
                                                                   const float win_pos_y) {
    if (m_sp_state ==
        SlicingProgressState::
            SP_PROGRESS /*|| (m_sp_state == SlicingProgressState::SP_COMPLETED && !m_sidebar_collapsed)*/) {
        if (m_text2 == "") {
            ...
            //	render_cancel_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
            // ProgressBarNotification::render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y,
            // -m_line_height, m_percentage, m_render_percentage); ImGui::SetCursorPosX(m_left_indentation);
            // ImGui::SetCursorPosY(0);
            // imgui.text(m_text2.c_str());
            // ProgressBarNotification::render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y, m_line_height,
            // m_percentage, m_render_percentage);
            // two lines text (what doesnt fit, wont show), one line bar
            ImGui::SetCursorPosX(m_left_indentation);
            ImGui::SetCursorPosY(m_line_height / 4);
        ...
        m_text1[m_endlines[0]] == ' ' ? 1 : 0)); imgui.text(line.c_str()); if (m_sidebar_collapsed && m_sp_state ==
        SlicingProgressState::SP_PROGRESS && m_export_possible) { ImVec2 text_size =
        ImGui::CalcTextSize(line.c_str()); render_hypertext(imgui, m_left_indentation + text_size.x + 4, m_line_height
        + m_line_height / 4, m_hypertext);
            }
            if (m_has_cancel_button)
                render_cancel_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
            render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
...
void NotificationManager::SlicingProgressNotification::render_bar(ImGuiWrapper &imgui,
                                                                  const float win_size_x,
                                                                  const float win_size_y,
                                                                  const float win_pos_x,
                                                                  const float win_pos_y) {
...
void NotificationManager::SlicingProgressNotification::render_cancel_button(ImGuiWrapper &imgui,
                                                                            const float win_size_x,
                                                                            const float win_size_y,
                                                                            const float win_pos_x,
                                                                            const float win_pos_y) {
...
void NotificationManager::SlicingProgressNotification::render_close_button(ImGuiWrapper &imgui,
                                                                           const float win_size_x,
                                                                           const float win_size_y,
                                                                           const float win_pos_x,
                                                                           const float win_pos_y) {
...
void NotificationManager::ProgressIndicatorNotification::render_cancel_button(ImGuiWrapper &imgui,
                                                                              const float win_size_x,
                                                                              const float win_size_y,
                                                                              const float win_pos_x,
                                                                              const float win_pos_y) {
...
void NotificationManager::ProgressIndicatorNotification::render_close_button(ImGuiWrapper &imgui,
                                                                             const float win_size_x,
                                                                             const float win_size_y,
                                                                             const float win_pos_x,
                                                                             const float win_pos_y) {
...
void NotificationManager::push_slicing_warning_notification(const std::string &text,
    ...
    auto notification = std::make_unique<NotificationManager::ObjectIDNotification>(data, m_id_provider,
                                                                                    m_evt_handler);
...
void NotificationManager::push_version_notification(NotificationType type,
                                                    NotificationLevel level,
                                                    const std::string &text,
                                                    const std::string &hypertext,
                                                    std::function<bool(wxEvtHandler *)> callback) {
...
void NotificationManager::push_exporting_finished_notification(const std::string &path,
...
void NotificationManager::push_upload_job_notification(
    ...
    // filename is created from boost::filesystem::path.string() which if created by path / "file" return \\ as folder
    // division. But could also contain / as folder division. Lets unite this into "/" only.
    ...
    NotificationData data{NotificationType::PrintHostUpload, NotificationLevel::ProgressBarNotificationLevel, 10,
                          text};
...
void NotificationManager::upload_job_notification_show_canceled(int id,
                                                                const std::string &filename,
                                                                const std::string &host) {
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::PrintHostUpload) {
...
void NotificationManager::upload_job_notification_show_error(int id,
                                                             const std::string &filename,
                                                             const std::string &host) {
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::PrintHostUpload) {
...
void NotificationManager::set_slicing_progress_percentage(const std::string &text,
                                                          float percentage,
                                                          bool main /*= true*/) {
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::SlicingProgress) {
            SlicingProgressNotification *spn = dynamic_cast<SlicingProgressNotification *>(notification.get());
            if (main) {
                if (spn->set_progress_state(SlicingProgressNotification::SlicingProgressState::SP_PROGRESS,
                                            percentage)) {
...
void NotificationManager::deactivate_loaded_hints() { HintDatabase::get_instance().uninit(); }
...
void NotificationManager::push_delayed_notification_data(
    std::unique_ptr<NotificationManager::PopNotification> notification,
    std::function<bool(void)> condition_callback,
    int64_t initial_delay,
    int64_t delay_interval) {
...
bool NotificationManager::update_notifications(GLCanvas3D &canvas) {
    ...
    for (auto it = m_waiting_notifications.begin(); it != m_waiting_notifications.end();) {
        ...
        if ((*it).remaining_time <= 0) {
            if ((*it).notification && (*it).condition_callback()) { // push notification, erase it from waiting list
                                                                    // (frame is scheduled by push)
    ...
    for (std::unique_ptr<PopNotification>& notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::ExportToRemovableFinished) {
            if (!on_removable) {
                ...
                notification->update( {old_data.type, old_data.level ,old_data.duration, std::string(),
    old_data.hypertext} ); } else { notification->close();
            }
            return;
        }
    }
