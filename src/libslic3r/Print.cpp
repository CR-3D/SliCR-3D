///|/ Copyright (c) Prusa Research 2016 - 2023 Lukáš Matěna @lukasmatena, Tomáš Mészáros @tamasmeszaros, Enrico Turri
///@enricoturri1966, Vojtěch Bubník @bubnikv, Pavel Mikuš @Godrak, Oleksandra Iushchenko @YuSanka, Lukáš Hejl
///@hejllukas, Filip Sykala @Jony01, Roman Beránek @zavorka, David Kocík @kocikdav
...
bool Print::invalidate_state_by_config_options(const ConfigOptionResolver & /* new_config */,
    ...
    for (const t_config_option_key &opt_key : opt_keys) {
        ...
        if (steps_gcode.find(opt_key) != steps_gcode.end()) {
            // These options only affect G-code export or they are just notes without influence on the generated
            // G-code, so there is nothing to invalidate.
            steps.emplace_back(psGCodeExport);
        } else if (steps_ignore.find(opt_key) != steps_ignore.end()) {
            // These steps have no influence on the G-code whatsoever. Just ignore them.
        } else if (opt_key == "complete_objects_one_skirt" || opt_key == "draft_shield" ||
                   opt_key == "min_skirt_length" || opt_key == "ooze_prevention" || opt_key == "skirts" ||
                   opt_key == "skirt_brim" || opt_key == "skirt_distance" || opt_key == "skirt_distance_from_brim" ||
                   opt_key == "skirt_extrusion_width" || opt_key == "skirt_height" ||
                   opt_key == "wipe_tower_rotation_angle") {
            steps.emplace_back(psSkirtBrim);
        } else if (opt_key == "bridge_precision" || opt_key == "filament_shrink" || opt_key == "nozzle_diameter" ||
                   opt_key == "resolution" ||
                   opt_key == "resolution_internal"
                   // Spiral Vase forces different kind of slicing than the normal model:
                   // In Spiral Vase mode, holes are closed and only the largest area contour is kept at each layer.
                   // Therefore toggling the Spiral Vase on / off requires complete reslicing.
                   || opt_key == "spiral_vase" || opt_key == "z_step") {
            osteps.emplace_back(posSlice);
        } else if (opt_key == "complete_objects" || opt_key == "filament_type" ||
                   ...
                   opt_key == "filament_max_wipe_tower_speed" || opt_key == "filament_minimal_purge_on_wipe_tower" ||
                   opt_key == "filament_cooling_initial_speed" || opt_key == "filament_cooling_final_speed" ||
                   opt_key == "filament_ramming_parameters" || opt_key == "filament_max_speed" ||
                   opt_key == "filament_max_volumetric_speed" || opt_key == "filament_multitool_ramming" ||
                   ...
                   || opt_key == "filament_use_fast_skinnydip" || opt_key == "filament_skinnydip_distance" ||
                   opt_key == "filament_melt_zone_pause" || opt_key == "filament_cooling_zone_pause" ||
                   opt_key == "filament_toolchange_temp" || opt_key == "filament_enable_toolchange_temp" ||
                   opt_key == "filament_enable_toolchange_part_fan" ||
                   opt_key == "filament_toolchange_part_fan_speed" || opt_key == "filament_dip_insertion_speed" ||
                   opt_key == "filament_dip_extraction_speed" // skinnydip params end
                   || opt_key == "first_layer_temperature" || opt_key == "gcode_flavor" ||
                   opt_key == "high_current_on_filament_swap" || opt_key == "priming_position" ||
                   ...
                   opt_key == "wipe_tower_extrusion_width" || opt_key == "wipe_tower_no_sparse_layers" ||
                   opt_key == "wipe_tower_extruder" || opt_key == "wipe_tower_per_color_wipe" ||
                   opt_key == "wipe_tower_speed" || opt_key == "wipe_tower_wipe_starting_speed" ||
                   opt_key == "wiping_volumes_extruders" || opt_key == "wiping_volumes_matrix" ||
                   opt_key == "parking_pos_retraction" || opt_key == "cooling_tube_retraction" ||
                   opt_key == "cooling_tube_length" || opt_key == "extra_loading_move" || opt_key == "travel_speed" ||
                   opt_key == "travel_speed_z" || opt_key == "z_offset") {
            steps.emplace_back(psWipeTower);
            steps.emplace_back(psSkirtBrim);
        } else if (opt_key == "filament_soluble") {
            steps.emplace_back(psWipeTower);
            // Soluble support interface / non-soluble base interface produces non-soluble interface layers below
            // soluble interface layers. Thus switching between soluble / non-soluble interface layer material may
            // require recalculation of supports.
            // FIXME Killing supports on any change of "filament_soluble" is rough. We should check for each object
            // whether that is necessary.
            osteps.emplace_back(posSupportMaterial);
        } else if (opt_key == "arc_fitting" || opt_key == "arc_fitting_resolution" ||
                   opt_key == "arc_fitting_tolerance" || opt_key == "min_layer_height" ||
                   opt_key == "max_layer_height" || opt_key == "filament_max_overlap" || opt_key == "wall_sequence" ||
...
std::set<uint16_t> Print::object_extruders(float z /*= -1*/) const { return object_extruders(m_objects, z); }
        ...
        if (object->has_support_material()) {
            ...
            if (z >= 0) {
                ...
                for (const SupportLayer *suppl : object->support_layers()) {
                    if (suppl->print_z - suppl->height - EPSILON < z && z < suppl->print_z + EPSILON &&
...
bool Print::has_infinite_skirt() const {
    return (m_config.draft_shield.value == dsEnabled &&
...
bool Print::has_skirt() const {
    return (m_config.skirt_height > 0 && m_config.skirts > 0) || this->has_infinite_skirt() ||
        m_config.draft_shield.value != dsDisabled;
...
bool Print::has_brim() const {
    return !this->m_brim.empty() ||
        std::any_of(m_objects.begin(), m_objects.end(), [](PrintObject *object) { return object->has_brim(); });
}
...
bool Print::sequential_print_horizontal_clearance_valid(const Print &print, Polygons *polygons) {
    ...
    for (const PrintObject *print_object : print.objects()) {
        const double object_grow = (print.config().complete_objects && !print_object->config().brim_per_object) ?
            dist_grow :
            std::max(dist_grow, print_object->config().brim_width.value);
        ...
        if (it_convex_hull == map_model_object_to_convex_hull.end()) {
            ...
            Polygons offs_ch2d = offset(ch2d,
                                        // Shrink the extruder_clearance_radius a tiny bit, so that if the object
                                        // arrangement algorithm placed the objects exactly by satisfying the
                                        // extruder_clearance_radius, this test will not trigger collision.
        ...
        if (it_convex_hull != map_model_object_to_convex_hull.end()) {
            ...
            const double z_diff =
                Geometry::rotation_diff_z(model_instance0->get_matrix(),
                                          print_object->instances().front().model_instance->get_matrix());
            ...
            for (const PrintInstance &instance : print_object->instances()) {
                ...
                // Convert the shift from the PrintObject's coordinates into ModelObject's coordinates by removing the
                // centering offset.
    ...
    if (!intersecting_idxs.empty()) {
        ...
        intersecting_idxs.erase(std::unique(intersecting_idxs.begin(), intersecting_idxs.end()),
                                intersecting_idxs.end());
...
static inline bool sequential_print_vertical_clearance_valid(const Print &print) {
    ...
    auto it = std::max_element(print_instances_ordered.begin(), print_instances_ordered.end(),
                               [](auto l, auto r) { return l->print_object->height() < r->print_object->height(); });
    return it == print_instances_ordered.end() ||
        (*it)->print_object->height() <= scale_(print.config().extruder_clearance_height.value);
}
...
double Print::get_object_first_layer_height(const PrintObject &object) const {
    ...
    if (object.config().first_layer_height.percent) {
        ...
        for (const PrintRegion &region : object.all_regions()) {
            PrintRegion::collect_object_printing_extruders(config(), object.config(), region.config(),
                                                           object_extruders);
        }
        ...
        for (uint16_t extruder_id : object_extruders) {
            ...
            object_first_layer_height = std::min(object_first_layer_height,
                                                 object.config().first_layer_height.get_abs_value(nozzle_diameter));
        }
    }
...
std::pair<PrintBase::PrintValidationError, std::string> Print::validate(std::vector<std::string> *warnings) const {
        ...
        for (auto it_a = extruders.begin(); it_a != extruders.end(); ++it_a)
            for (auto it_b = std::next(it_a); it_b != extruders.end(); ++it_b)
                if (std::abs(m_config.bed_temperature.get_at(*it_a) - m_config.bed_temperature.get_at(*it_b)) > 15 ||
                    std::abs(m_config.first_layer_bed_temperature.get_at(*it_a) -
                             m_config.first_layer_bed_temperature.get_at(*it_b)) > 15) {
    ...
    if (m_objects.empty())
        return {PrintBase::PrintValidationError::pveWrongPosition,
                _u8L("All objects are outside of the print volume.")};
    ...
    if (m_config.complete_objects /*|| m_config.parallel_objects_step > 0*/) {
        if (!sequential_print_horizontal_clearance_valid(*this,
            ...
            return {PrintBase::PrintValidationError::pveWrongPosition,
                    _u8L("Some objects are too close; your extruder will collide with them.")};
        if (m_config.complete_objects && !sequential_print_vertical_clearance_valid(*this))
            return {PrintBase::PrintValidationError::pveWrongPosition,
                    _u8L("Some objects are too tall and cannot be printed without extruder collisions.")};
    } else
    ...
    if (m_config.avoid_crossing_perimeters && m_config.avoid_crossing_curled_overhangs) {
        return {PrintBase::PrintValidationError::pveWrongSettings,
                _u8L("Avoid crossing perimeters option and avoid crossing curled overhangs option cannot be both "
                     "enabled together.")};
    }
    ...
    if (m_config.spiral_vase) {
        ...
        if (m_objects.front()->all_regions().size() > 1)
            return {PrintBase::PrintValidationError::pveWrongSettings,
                    _u8L("The Spiral Vase option can only be used when printing single material objects, and without "
                         "any modifiers that may break the vase.")};
    }
    ...
    auto layer_height_profile =
        [this, &layer_height_profiles](const size_t print_object_idx) -> const std::vector<coordf_t> & {
        ...
        if (auto layers = generate_object_layers(print_object.slicing_parameters(),
            ...
            return {PrintBase::PrintValidationError::pveWrongPosition,
                    ...
                    0.5 * (layers[layers.size() - 2] + layers.back()) > this->config().max_print_height + EPSILON ?
                        format(_u8L("The object %1% exceeds the maximum build volume height."),
                               print_object.model_object()->name) :
                        format(_u8L("While the object %1% itself fits the build volume, its last layer exceeds the "
                                    "maximum build volume height."),
                               print_object.model_object()->name) +
                            " " +
                            _u8L("You might want to reduce the size of your model or change current print settings "
                                 "and retry.")};
        }
    }
    ...
    for (size_t print_object_idx = 0; print_object_idx < m_objects.size(); ++print_object_idx)
        if (const PrintObject &print_object = *m_objects[print_object_idx]; print_object.has_support_material() &&
            print_object.config().support_material_style.value == smsOrganic &&
            print_object.model_object()->has_custom_layering()) {
            if (const std::vector<coordf_t> &layers = layer_height_profile(print_object_idx); !layers.empty())
                if (!check_object_layers_fixed(print_object.slicing_parameters(), layers))
                    return {PrintBase::PrintValidationError::pveWrongSettings,
                            _u8L("Variable layer height is not supported with Organic supports.")};
        }
        ...
        for (const uint16_t& extruder_idx : extruders) {
            ...
            if (nozzle_diam - EPSILON > first_nozzle_diam || nozzle_diam + EPSILON < first_nozzle_diam
             || std::abs((filament_diam-first_filament_diam)/first_filament_diam) > 0.1)
                return { PrintBase::PrintValidationError::pveWrongSettings, _u8L("The wipe tower is only supported if
        all extruders have the same nozzle diameter " "and use filaments of the same diameter.") };
        }
*/

        if (m_config.gcode_flavor != gcfRepRap && m_config.gcode_flavor != gcfSprinter &&
            ...
            return {PrintBase::PrintValidationError::pveWrongSettings,
                    _u8L("The Wipe Tower is currently only supported for the Marlin, Klipper, RepRap/Sprinter and "
                         "Repetier G-code flavors.")};
        if (!m_config.use_relative_e_distances)
            return {PrintBase::PrintValidationError::pveWrongSettings,
                    _u8L("The Wipe Tower is currently only supported with the relative extruder addressing "
                         "(use_relative_e_distances=1).")};
        if (m_config.ooze_prevention && m_config.single_extruder_multi_material)
            return {PrintBase::PrintValidationError::pveWrongSettings,
                    _u8L("Ooze prevention is currently not supported with the wipe tower enabled.")};
        if (m_config.use_volumetric_e)
            return {PrintBase::PrintValidationError::pveWrongSettings,
                    _u8L("The Wipe Tower currently does not support volumetric E (use_volumetric_e=0).")};
        if (m_config.complete_objects && extruders.size() > 1)
            return {PrintBase::PrintValidationError::pveWrongSettings,
                    _u8L("The Wipe Tower is currently not supported for multimaterial sequential prints.")};
        ...
        if (m_objects.size() > 1) {
            ...
            for (size_t i = 1; i < m_objects.size(); ++i) {
                ...
                if (std::abs(slicing_params.first_print_layer_height - slicing_params0.first_print_layer_height) >
                    ...
                    return {PrintBase::PrintValidationError::pveWrongSettings,
                            _u8L("The Wipe Tower is only supported for multiple objects if they have equal layer "
                                 "heights")};
                if (slicing_params.raft_layers() != slicing_params0.raft_layers())
                    return {PrintBase::PrintValidationError::pveWrongSettings,
                            _u8L("The Wipe Tower is only supported for multiple objects if they are printed over an "
                                 "equal number of raft layers")};
                if (object->config().support_material_contact_distance_type !=
                        m_objects.front()->config().support_material_contact_distance_type ||
                    object->config().support_material_contact_distance.value !=
                        m_objects.front()->config().support_material_contact_distance.value ||
                    object->config().support_material_bottom_contact_distance.value !=
                        m_objects.front()->config().support_material_bottom_contact_distance.value ||
                    slicing_params0.gap_object_support != slicing_params.gap_object_support ||
                    slicing_params0.gap_support_object != slicing_params.gap_support_object)
                    return {PrintBase::PrintValidationError::pveWrongSettings,
                            _u8L("The Wipe Tower is only supported for multiple objects if they are printed with the "
                ...
                for (size_t idx_object = 0; idx_object < m_objects.size(); ++idx_object) {
                    ...
                    while (i < layer_height_profiles[idx_object].size() &&
                        ...
                        if (std::abs(layer_height_profiles[idx_object][i] -
                            ...
                            return {PrintBase::PrintValidationError::pveWrongSettings,
                                    _u8L("The Wipe tower is only supported if all objects have the same variable "
                                         "layer height")};
                        ++i;
                    }
                }
        ...
        for (PrintObject *object : m_objects) {
            if (object->has_support_material()) {
                if ((object->config().support_material_extruder == 0 ||
                    ...
                    // The object has some form of support and either support_material_extruder or
                    // support_material_interface_extruder will be printed with the current tool without a forced tool
                    // change. Play safe, assert that all object nozzles are of the same diameter.
                    return {PrintBase::PrintValidationError::pveWrongSettings,
                            _u8L("Printing with multiple extruders of differing nozzle diameters. "
                                 "If support is to be printed with the current extruder (support_material_extruder "
                                 "== 0 or support_material_interface_extruder == 0), "
                ...
                if (this->has_wipe_tower() && object->config().support_material_style != smsOrganic) {
                    if (object->config().support_material_contact_distance_type.value == zdNone) {
                        ...
                        if (!object->config().support_material_synchronize_layers)
                            return {PrintBase::PrintValidationError::pveWrongSettings,
                                    _u8L("For the Wipe Tower to work with the soluble supports, the support layers "
                        ...
                        if (object->config().support_material_extruder != 0 ||
                            object->config().support_material_interface_extruder != 0)
                            return {PrintBase::PrintValidationError::pveWrongSettings,
                                    _u8L("The Wipe Tower currently supports the non-soluble supports only if they "
                ...
                if (object->config().support_material_style.value == smsOrganic) {
                    ...
                    if (object->config().support_tree_tip_diameter < extrusion_width - EPSILON)
                        return {PrintBase::PrintValidationError::pveWrongSettings,
                                _u8L("Organic support tree tip diameter must not be smaller than support material "
                                     "extrusion width.")};
                    if (object->config().support_tree_branch_diameter < 2. * extrusion_width - EPSILON)
                        return {PrintBase::PrintValidationError::pveWrongSettings,
                                _u8L("Organic support branch diameter must not be smaller than 2x support material "
                                     "extrusion width.")};
                    if (object->config().support_tree_branch_diameter < object->config().support_tree_tip_diameter)
                        return {PrintBase::PrintValidationError::pveWrongSettings,
                                _u8L("Organic support branch diameter must not be smaller than support tree tip "
                                     "diameter.")};
                }
            }
            ...
            for (const PrintRegion &region : object->all_regions()) {
                ...
                for (uint16_t extruder_id : object_extruders) {
                    ...
                    if (max_layer_height < EPSILON || !config().max_layer_height.is_enabled())
                    ...
                    // if (max_layer_height > nozzle_diameter) return { PrintBase::PrintValidationError::pveWrongSettings,
                    // _u8L("Max layer height can't be greater than nozzle diameter") };
                    double skirt_width =
                        Flow::new_from_config_width(frPerimeter,
                                                    ...
                                                    *Flow::extrusion_spacing_option("skirt", m_default_region_config),
                                                    ...
                                                    print_first_layer_height, 1,
                                                    0 // don't care, all i want if width from width
                                                    )
                            .width();
                    // check first layer layer_ranges

                    if (object->shared_regions()->layer_ranges.front().layer_height_range.first <
                        ...
                        for (auto tuple : std::vector<std::pair<double, const char *>>{
                                 ...
                                 {region.width(FlowRole::frExternalPerimeter, true, *object),
                                  "perimeter extrusion width"},
                                 {region.width(FlowRole::frInfill, true, *object), "infill extrusion width"},
                                 {region.width(FlowRole::frSolidInfill, true, *object),
                                  "solid infill extrusion width"},
                                 {region.width(FlowRole::frTopSolidInfill, true, *object),
                                  "top solid infill extrusion width"},
                             })
                    ...
                    if (object->shared_regions()->layer_ranges.front().layer_height_range.second > layer_height) {
                        ...
                        for (auto tuple : std::vector<std::pair<double, const char *>>{
                                 ...
                                 {region.width(FlowRole::frExternalPerimeter, false, *object),
                                  "perimeter extrusion width"},
                                 {region.width(FlowRole::frInfill, false, *object), "infill extrusion width"},
                                 {region.width(FlowRole::frSolidInfill, false, *object),
                                  "solid infill extrusion width"},
                                 {region.width(FlowRole::frTopSolidInfill, false, *object),
                                  "top solid infill extrusion width"},
                             })
    ...
    }
    {
        ...
        if (m_config.use_relative_e_distances) {
            ...
            //    return { PrintBase::PrintValidationError::pveWrongSettings, _u8L("Relative extruder addressing
            //    requires resetting the extruder position at each layer to prevent loss of floating point accuracy.
            //    Add \"G92 E0\" to layer_gcode.") };
        } else {
            if (before_layer_gcode_resets_extruder)
                return {PrintBase::PrintValidationError::pveWrongSettings,
                        _u8L("\"G92 E0\" was found in before_layer_gcode, which is incompatible with absolute "
                             "extruder addressing.")};
            else if (layer_gcode_resets_extruder)
                return {PrintBase::PrintValidationError::pveWrongSettings,
                        _u8L("\"G92 E0\" was found in layer_gcode, which is incompatible with absolute extruder "
                             "addressing.")};
        }
    }
...
Flow Print::brim_flow(size_t extruder_id, const PrintObjectConfig &brim_config) const {
    ...
    return Flow::new_from_config_width(frPerimeter, *Flow::extrusion_width_option("brim", tempConf),
                                       *Flow::extrusion_spacing_option("brim", tempConf),
                                       (float) m_config.nozzle_diameter.get_at(extruder_id),
                                       (float) get_min_first_layer_height(),
                                       (extruder_id < m_config.nozzle_diameter.size()) ?
                                           brim_config.get_computed_value("filament_max_overlap", extruder_id) :
                                           1);
}
...
Flow Print::skirt_flow(size_t extruder_id) const {
    ...
    // Use the actual extruder's nozzle diameter instead of the maximum across objects.
    const float nozzle_diameter = (float) m_config.nozzle_diameter.get_at(extruder_id);
    // Honor per-extruder filament overlap limits if available.
    const float filament_overlap = (extruder_id < m_config.nozzle_diameter.size()) ?
        (float) m_default_object_config.get_computed_value("filament_max_overlap", extruder_id) :
        1.f;

    // m_default_object_config is the minimal configuration needed for extrusion_option.
    return Flow::new_from_config_width(frPerimeter, *Flow::extrusion_width_option("skirt", m_default_region_config),
                                       *Flow::extrusion_spacing_option("skirt", m_default_region_config),
                                       nozzle_diameter, (float) get_min_first_layer_height(), filament_overlap);
}
...
public:
    ...
    void use(const ExtrusionLoop &loop) override { assert(loop.is_counter_clockwise() == ccw); }
};
#endif
...
void Print::process() {
    ...
    Slic3r::parallel_for(size_t(0), m_objects.size(), [this](const size_t idx) { m_objects[idx]->make_perimeters(); });
#ifdef _DEBUG
    for (PrintObject *obj : m_objects)
...
#endif
    secondary_status_counter_reset();
    Slic3r::parallel_for(size_t(0), m_objects.size(), [this](const size_t idx) { m_objects[idx]->infill(); });
    secondary_status_counter_reset();
    Slic3r::parallel_for(size_t(0), m_objects.size(), [this](const size_t idx) { m_objects[idx]->ironing(); });
    ...
    Slic3r::parallel_for(size_t(0), m_objects.size(),
                         [this](const size_t idx) { m_objects[idx]->generate_support_material(); });
    secondary_status_counter_reset();
    Slic3r::parallel_for(size_t(0), m_objects.size(), [this](const size_t idx) {
        PrintObject &obj = *m_objects[idx];
        m_objects[idx]->estimate_curled_extrusions();
    });
    secondary_status_counter_reset();
    Slic3r::parallel_for(size_t(0), m_objects.size(),
                         [this](const size_t idx) { m_objects[idx]->calculate_overhanging_perimeters(); });
    ...
    // simplify / make arc fitting
    {
        ...
        public:
            ...
            virtual void use(ExtrusionLoop &loop) override { loops.push_back(&loop); }
        } get_loops;
#endif
    ...
    if (something_done)
        this->set_status(printstep_2_percent[PrintStep::psGCodeExport], L("Slicing done"),
    ...
    if (this->set_started(psSkirtBrim)) {
        ...
        for (PrintObject *obj : m_objects) {
            ...
            for (std::vector<PrintObject *> &obj_group : obj_groups) {
                bool same_first_layer_extrusion_width =
                    obj_group.front()->config().first_layer_extrusion_width.is_enabled() ==
                    obj->config().first_layer_extrusion_width.is_enabled();
                if (same_first_layer_extrusion_width &&
                    obj_group.front()->config().first_layer_extrusion_width.is_enabled()) {
                    same_first_layer_extrusion_width = obj_group.front()->config().first_layer_extrusion_width.value ==
                        obj->config().first_layer_extrusion_width.value;
                }
        ...
        if (obj_groups.size() > 1 || brim_per_object ||
            (!obj_groups.empty() &&
             (has_brim_patch(obj_groups.front(), ModelVolumeType::BRIM_PATCH) ||
              has_brim_patch(obj_groups.front(), ModelVolumeType::BRIM_NEGATIVE)))) {
            for (std::vector<PrintObject *> &obj_group : obj_groups) {
                for (const PrintObject *object : obj_group) {
                    if (!object->m_layers.empty()) {
                        for (const PrintInstance &pt : object->m_instances) {
                            ...
                            brim_area.insert(brim_area.end(), object->m_layers.front()->lslices().begin(),
        ...
        for (std::vector<PrintObject *> &obj_group : obj_groups) {
            ...
            if (brim_config.brim_width > 0 || brim_config.brim_width_interior > 0 ||
                has_brim_patch(obj_group, ModelVolumeType::BRIM_PATCH)) {
                ...
                if (brim_config.brim_per_object) {
                    for (PrintObject *obj : obj_group) {
                        ...
                        Flow flow = this->brim_flow(set_extruders.empty() ?
                                                        get_print_region(0).config().perimeter_extruder - 1 :
                                                        *set_extruders.begin(),
                                                    obj->config());
                        // if complete objects
                        if (config().complete_objects || config().parallel_objects_step.value > 0) {
                            ...
                            make_brim_patch(*this, flow, obj->get_brim_patch(ModelVolumeType::BRIM_PATCH), brim_area,
                                            obj->m_brim);
                            obj->m_instances = copies;
                        } else {
                            ...
                            for (const PrintInstance &instance : copies) {
                                ...
                                make_brim_patch(*this, flow, obj->get_brim_patch(ModelVolumeType::BRIM_PATCH),
                                                brim_area, entity_brim);
                    ...
                    for (PrintObject *obj : obj_group) {
                        ...
                        for (const PrintInstance &instance : obj->instances()) {
                            ExtrusionEntityCollection entity_brim;
                            make_brim_patch(*this, flow, obj->get_brim_patch(ModelVolumeType::BRIM_PATCH, &instance),
                                            brim_area, entity_brim);
                            obj->m_brim.append(std::move(entity_brim));
                        }
                    }
...
void Print::_make_skirt(const PrintObjectPtrs &objects,
                        ...
                        std::optional<ExtrusionEntityCollection> &out_first_layer) {
    assert(out.empty());
    ...
    for (const PrintObject *object : objects) {
        ...
        if (config().skirt_distance_from_brim) {
            ...
            if (!object->support_layers().empty() &&
                object->support_layers().front()->print_z == object->m_layers[0]->print_z) {
            ...
            if (has_brim_patch(*object, ModelVolumeType::BRIM_PATCH)) {
                ...
                for (Polygon &poly :
                     object->get_brim_patch(ModelVolumeType::BRIM_PATCH, &object->instances().front())) {
                    // remove shift
    ...
    float distance = float(scale_(m_config.skirt_distance.value) -
                           this->skirt_flow(extruders[extruders.size() - 1]).spacing() / 2.);
        ...
        eloop.paths.emplace_back(
            ExtrusionAttributes{ExtrusionRole::Skirt,
                                ExtrusionFlow{
                                    ...
                                    flow.width(),
                                    float(
                                        get_min_first_layer_height()) // this will be overridden at G-code export time
                                }},
            false);
    ...
    for (Polygon &poly :
         offset(convex_hull,
                distance + 0.5f * float(this->skirt_flow(extruders[extruders.size() - 1]).scaled_spacing()),
...
Polygons Print::first_layer_islands() const {
    ...
    for (PrintObject *object : m_objects) {
        ...
        if (!object->support_layers().empty())
            ...
            object->support_layers().front()->support_fills.polygons_covered_by_width(object_islands,
    ...
    if (this->set_started(psAlertWhenSupportsNeeded)) {
        ...
        auto issue_to_alert_message = [](SupportSpotsGenerator::SupportPointCause cause, bool critical) {
            ...
            case SupportSpotsGenerator::SupportPointCause::FloatingExtrusion:
                if (critical) {
                    // TRN Alert when support is needed. Describes that the print has large overhang area which will
                    // print badly or not print at all.
                    message = _u8L("Collapsing overhang");
                } else {
                    // TRN Alert when support is needed. Describes extrusions that are not supported enough and come
                    // out curled or loose.
            ...
            case SupportSpotsGenerator::SupportPointCause::SeparationFromBed:
                ...
                break;
            // TRN Alert when support is needed. Describes that the object has part that is not connected to the bed
            // and will not print at all without supports.
        ...
        // TRN this translation rule is used to translate lists of uknown size on single line. The first argument is
        // element of the list, the second argument may be element or rest of the list. For most languages, this does
        // not need translation, but some use different separator than comma and some use blank space in front of the
        // separator.
        auto single_line_list_rule = L("%1%, %2%");
        auto multiline_list_rule = "%1%\n%2%";

        auto elements_to_translated_list = [](const std::vector<std::string> &translated_elements,
                                              std::string expansion_rule) {
            if (expansion_rule.find("%1%") == expansion_rule.npos ||
                expansion_rule.find("%2%") == expansion_rule.npos) {
        ...
        for (const PrintObject *object : m_objects) {
            ...
            if (!object->has_support() &&
                ...
                if (object->m_shared_regions->generated_support_points.has_value()) {
                    SupportSpotsGenerator::SupportPoints supp_points = object->m_shared_regions
                                                                           ->generated_support_points->support_points;
...
bool Print::has_wipe_tower() const {
    return !m_config.spiral_vase.value && m_config.wipe_tower.value && m_config.nozzle_diameter.size() > 1;
}
...
const WipeTowerData &Print::wipe_tower_data(const ConfigBase *config, double nozzle_diameter) const {
    ...
    if (!is_step_done(psWipeTower) && config != &this->m_config) {
        ...
        float unscaled_brim_width = config->option<ConfigOptionFloatOrPercent>("wipe_tower_brim_width")
        ...
        // first_layer_height =
        // default_object_config().first_layer_height.get_abs_value(config->option("nozzle_diameter")->get_float(0));
        first_layer_height = config->option<ConfigOptionFloatOrPercent>("first_layer_height")
                                 ->get_abs_value(config->option("nozzle_diameter")->get_float(0));
        //} else {
...
void Print::_make_wipe_tower() {
    ...
    // see https://github.com/prusa3d/PrusaSlicer/issues/607
    {
        ...
        if (idx_begin != size_t(-1)) {
            ...
            double wipe_tower_new_layer_print_z_first = m_wipe_tower_data.tool_ordering.layer_tools()[idx_begin]
                                                            .print_z;
            SupportLayerPtrs::const_iterator it_layer = m_objects.front()->edit_support_layers().begin();
            for (; it_layer != m_objects.front()->edit_support_layers().end() &&
                 (*it_layer)->print_z - EPSILON < wipe_tower_new_layer_print_z_first;
                 ++it_layer)
                ;
            // Find the stopper of the sequence of wipe tower layers, which do not have a counterpart in an object or
            // a support layer.
    ...
    m_wipe_tower_data.priming = Slic3r::make_unique<std::vector<WipeTower::ToolChangeResult>>(
        wipe_tower.prime((float) get_min_first_layer_height(), m_wipe_tower_data.tool_ordering.all_extruders(),
                         false));
                ...
                if ((first_layer && extruder_id == m_wipe_tower_data.tool_ordering.all_extruders().back()) ||
                    ...
                    if (m_config.wipe_advanced) {
                        ...
                        if (m_config.wipe_advanced_algo.value == waLinear) {
                            ...
                            BOOST_LOG_TRIVIAL(info) << current_extruder_id << " -> " << extruder_id << " will use "
                                                    << volume_to_wipe << " mm3\n";
                            BOOST_LOG_TRIVIAL(info) << " calculus : " << m_config.wipe_advanced_nozzle_melted_volume
                                                    << " + " << m_config.wipe_advanced_multiplier.value << " * ( "
                                                    << pigmentBef << " - " << pigmentAft << " )\n";
                            BOOST_LOG_TRIVIAL(info)
                                << "    = " << m_config.wipe_advanced_nozzle_melted_volume << " + "
                                << (m_config.wipe_advanced_multiplier.value * (pigmentBef - pigmentAft)) << "\n";
                        } else if (m_config.wipe_advanced_algo.value == waQuadra) {
                            volume_to_wipe += m_config.wipe_advanced_multiplier.value * (pigmentBef - pigmentAft) +
                                m_config.wipe_advanced_multiplier.value * (pigmentBef - pigmentAft) *
                                    (pigmentBef - pigmentAft) * (pigmentBef - pigmentAft);
                            BOOST_LOG_TRIVIAL(info) << "advanced wiping (quadra) ";
                            BOOST_LOG_TRIVIAL(info) << current_extruder_id << " -> " << extruder_id << " will use "
                                                    << volume_to_wipe << " mm3\n";
                            BOOST_LOG_TRIVIAL(info)
                                << " calculus : " << m_config.wipe_advanced_nozzle_melted_volume << " + "
                                << m_config.wipe_advanced_multiplier.value << " * ( " << pigmentBef << " - "
                                << pigmentAft << " ) + " << m_config.wipe_advanced_multiplier.value << " * ( "
                            ...
                            volume_to_wipe += m_config.wipe_advanced_multiplier.value * (0.5 + pigmentBef) /
                                (0.5 + pigmentAft);
                            ...
                            BOOST_LOG_TRIVIAL(info)
                                << " calculus : " << m_config.wipe_advanced_nozzle_melted_volume << " + "
                                << m_config.wipe_advanced_multiplier.value << " * ( 0.5 + " << pigmentBef
                                << " ) / ( 0.5 + " << pigmentAft << " )\n";
                            BOOST_LOG_TRIVIAL(info)
                                << "    = " << m_config.wipe_advanced_nozzle_melted_volume << " + "
                                << (m_config.wipe_advanced_multiplier.value * (0.5 + pigmentBef) / (0.5 + pigmentAft))
                                << "\n";
                        }
                    }
                    ...
                    volume_to_wipe = layer_tools.wiping_extrusions_nonconst()
                                         .mark_wiping_extrusions(*this, layer_tools, current_extruder_id, extruder_id,
                                                                 volume_to_wipe);
                    ...
                    wipe_tower.plan_toolchange((float) layer_tools.print_z,
                                               (float) layer_tools.wipe_tower_layer_height, current_extruder_id,
                                               extruder_id, volume_to_wipe);
    ...
    if (m_wipe_tower_data.tool_ordering.back().wipe_tower_partitions > 0) {
        ...
        if (wipe_tower.layer_finished()) {
            // The wipe tower is printed to the top of the print and it has no space left for the final extruder
            // purge. Lift Z to the next layer.
...
// Generate a recommended G-code output file name based on the format template, default extension, and template
// parameters (timestamps, object placeholders derived from the model, current placeholder prameters and print
// statistics. Use the final print statistics if available, or just keep the print statistics placeholders if not
// available yet (before G-code is finalized).
...
DynamicConfig PrintStatistics::config() const {
    ...
    //    config.set_key_value("printing_extruders",        new
    //    ConfigOptionInts(std::vector<int>(this->printing_extruders.begin(), this->printing_extruders.end())));
