...
///Roman Beránek @zavorka, Lukáš Matěna @lukasmatena, Vojtěch Král @vojtechkral
...
static Polygons contours_simplified(const Vec2i32 &grid_size,
        ...
        for (;;) {
            ...
            for (auto it = line_range.first; it != line_range.second; ++it) {
                ...
                if (i_next == -1) {
                    ...
                    // This is a corner, where two lines meet exactly. Pick the line, which encloses a smallest angle
    ...
    for (Polygon &poly : out) {
        ...
        // Shrink the contour slightly, so if the same contour gets discretized and simplified again, one will get the
        // same result. Remove collinear points.
        Points pts;
...
PrintObjectSupportMaterial::PrintObjectSupportMaterial(const PrintObject *object,
                                                       std::shared_ptr<SlicingParameters> slicing_params)
...
#define assert_z(X) assert(true)
    ...
    // may get merged and trimmed by this->generate_base_layers() if the support layers are not synchronized with
    // object layers.
    std::vector<Polygons> layer_support_areas;
    SupportGeneratorLayersPtr bottom_contacts =
        this->bottom_contact_layers_and_layer_support_areas(object, top_contacts, buildplate_covered, layer_storage,
                                                            layer_support_areas);
    ...
    SupportGeneratorLayersPtr intermediate_layers = this->raft_and_intermediate_support_layers(object,
                                                                                               bottom_contacts,
                                                                                               top_contacts,
                                                                                               layer_storage);
    ...
    this->trim_support_layers_by_object(object, top_contacts, m_slicing_params->gap_support_object,
                                        ...
                                        m_support_params.gap_xy); // m_slicing_params->soluble_interface ? 0.
    ...
    auto [interface_layers,
          base_interface_layers] = FFFSupport::generate_interface_layers(*m_object_config, m_support_params,
                                                                         bottom_contacts, top_contacts, empty_layers,
    ...
    // If raft is to be generated, the 1st top_contact layer will contain the 1st object layer silhouette with holes
    // filled. There is also a 1st intermediate layer containing bases of support columns. Inflate the bases of the
    // support columns and create the raft base under the object.
    SupportGeneratorLayersPtr raft_layers = FFFSupport::generate_raft_base(object, m_support_params,
                                                                           *m_slicing_params, top_contacts,
        ...
        for (int i = 0; i < int(layers_sorted.size());) {
            ...
            for (; j < layers_sorted.size() && layers_sorted[j]->print_z <= zmax; ++j)
                if (!layers_sorted[j]->polygons.empty())
                    empty = false;
            if (!empty) {
                export_print_z_polygons_to_svg(debug_out_path("support-%d-%lf.svg", iRun, layers_sorted[i]->print_z)
                                                   .c_str(),
                                               layers_sorted.data() + i, j - i);
                export_print_z_polygons_and_extrusions_to_svg(debug_out_path("support-w-fills-%d-%lf.svg", iRun,
                                                                             layers_sorted[i]->print_z)
                                                                  .c_str(),
                                                              layers_sorted.data() + i, j - i,
                                                              *object.support_layers()[layer_id]);
                ++layer_id;
...
public:
    SupportGridPattern(
        ...
        // Trimming polygons, to trim the stretched support islands. support_polygons were already trimmed with
        // trimming_polygons.
                ...
                if (!samples_inside.empty()) {
                    ...
                    for (size_t i_contour = 0; i_contour <= island.holes.size(); ++i_contour) {
                        ...
                        for (; i != contour.points.end(); j = i++) {
                            // FIXME this test is not numerically robust. Particularly, it does not handle horizontal
                            // segments at y == point(1) well.
                            ...
                            for (auto &sample_inside : samples_inside) {
                                if (((*i)(1) > sample_inside.first(1)) != ((*j)(1) > sample_inside.first(1))) {
                                    ...
                                    double x2 = (double) (*i)(0) +
                                        (double) ((*j)(0) - (*i)(0)) * (double) (sample_inside.first(1) - (*i)(1)) /
                                            (double) ((*j)(1) - (*i)(1));
                                    if (x1 < x2)
...
#ifdef SLIC3R_DEBUG
            SVG::export_expolygons(debug_out_path("extract_support_from_grid_trimmed-%s-%d-%d-%lf.svg", step_name,
                                                  iRun, layer_id, print_z),
                                   {
                                       {{diff_ex(expand(*m_support_polygons, closing_distance),
                                                 closing(*m_support_polygons, closing_distance,
...
#endif /* SLIC3R_DEBUG */
            // FIXME do we want to trim with the object here? On one side the columns will be thinner, on the other
            // side support interfaces may disappear for snug supports.
            //  return diff(smooth_outward(closing(*m_support_polygons, closing_distance,
            //  SUPPORT_SURFACES_OFFSET_PARAMETERS), smoothing_distance), *m_trimming_polygons);
    ...
    static std::vector<unsigned char> dilate_trimming_region(const std::vector<unsigned char> &trimming,
        ...
        for (int r = 1; r + 1 < grid_size.y(); ++r)
            for (int c = 1; c + 1 < grid_size.x(); ++c) {
                ...
                //  dilated[addr] = trimming[addr] != 0 && trimming[addr - 1] != 0 && trimming[addr + 1] != 0 &&
                //  trimming[addr - m_grid_size.x()] != 0 && trimming[addr + m_grid_size.x()] != 0; 8-neighborhood
...
static bool has_bridging_perimeters(const ExtrusionEntityCollection &perimeters) { // FIXME
...
static bool has_bridging_extrusions(const Layer &layer) {
    for (const LayerRegion *region : layer.regions()) {
        if (SupportMaterialInternal::has_bridging_perimeters(region->perimeters()))
            return true;
        if (region->fill_surfaces().has(stPosBottom | stDensSolid | stModBridge) &&
            has_bridging_fills(region->fills()))
            return true;
    }
    return false;
}

static inline void collect_bridging_perimeter_areas(const ExtrusionLoop &loop,
    ...
    for (const ExtrusionPath &ep : loop.paths)
        if (ep.role().has(ExtrusionRole::OverhangPerimeter) && !ep.polyline.empty()) {
            ...
            if (ep.is_closed()) {
                ...
                // Offset the polyline.
                polygons_append(out,
                                offset(Polygon(ep.polyline.to_polyline().points), exp,
                                       SUPPORT_SURFACES_OFFSET_PARAMETERS));
            }
        }
}
static void collect_bridging_perimeter_areas(const ExtrusionEntitiesPtr &perimeters,
                                             const float expansion_scaled,
                                             Polygons &out) {
    for (const ExtrusionEntity *ee : perimeters) {
        if (ee->is_collection()) {
            collect_bridging_perimeter_areas(static_cast<const ExtrusionEntityCollection *>(ee)->entities(),
                                             expansion_scaled, out);
        } else if (ee->is_loop())
...
struct SupportAnnotations
{
    SupportAnnotations(const PrintObject &object, const std::vector<Polygons> &buildplate_covered)
        : enforcers_layers(), blockers_layers(), buildplate_covered(buildplate_covered) {
...
static inline std::tuple<Polygons, Polygons, Polygons, float> detect_overhangs(const Layer &layer,
    ...
    if (layer_id == 0) {
        ...
        // Cache support trimming polygons derived from lower layer polygons, possible merged with "on build plate
        // only" trimming polygons.
        auto slices_margin_update = [&slices_margin, &lower_layer, &lower_layer_polygons, buildplate_only,
                                     has_enforcer, &annotations, layer_id,
                                     resolution](float slices_margin_offset, float no_interface_offset) {
            if (slices_margin.offset != slices_margin_offset) {
                ...
                slices_margin.polygons = (slices_margin_offset == 0.f) ?
                    ...
                    // What is the purpose of no_interface_offset? Likely to not trim the contact layer by lower layer
                    // regions that are too thin to extrude?
                ...
                if (buildplate_only && !annotations.buildplate_covered[layer_id].empty()) {
                    ...
                    slices_margin.polygons = ensure_valid(union_(slices_margin.polygons,
                                                                 annotations.buildplate_covered[layer_id]),
                                                          resolution);
                }
            }
        };
        ...
        no_interface_offset =
            std::accumulate(layer.regions().begin(), layer.regions().end(), FLT_MAX,
        ...
        for (LayerRegion *layerm : layer.regions()) {
            ...
            for (auto &srf : layerm->slices().surfaces)
            ...
            if (lower_layer_offset == 0.f) {
                ...
                diff_polygons = opening_ex(
                    ...
                    0.1f * fw);
#else
                diff_polygons = diff_ex(layerm_expolygons,
                                        expand_ex(lower_layer_polygons, lower_layer_offset,
                                                  SUPPORT_SURFACES_OFFSET_PARAMETERS));
#endif
                if (buildplate_only && !annotations.buildplate_covered[layer_id].empty()) {
                    // Don't support overhangs above the top surfaces.
                ...
                if (!diff_polygons.empty()) {
                    ...
                    diff_polygons = diff_ex(intersection_ex(offset_ex(diff_polygons, lower_layer_offset,
                                                                      SUPPORT_SURFACES_OFFSET_PARAMETERS),
                                                            layerm_expolygons),
                                            lower_layer_polygons);
                }
            ...
            if (!annotations.blockers_layers.empty() && !annotations.blockers_layers[layer_id].empty()) {
                ...
                diff_polygons = diff_ex(diff_polygons,
                                        offset_ex(union_ex(annotations.blockers_layers[layer_id]),
        ...
        if (has_enforcer)
            if (const Polygons &enforcer_polygons_src = annotations.enforcers_layers[layer_id];
                !enforcer_polygons_src.empty()) {
    ...
    return std::make_tuple(std::move(overhang_polygons), std::move(contact_polygons), std::move(enforcer_polygons),
                           no_interface_offset);
}

// Allocate one, possibly two support contact layers.
// For "thick" overhangs, one support layer will be generated to support normal extrusions, the other to support the
// "thick" extrusions.
static inline std::pair<SupportGeneratorLayer *, SupportGeneratorLayer *> new_contact_layer(
    ...
    if (layer_id == 0) {
        ...
        // FIXME there may be a need for a single layer support, then one may decide to print it either as a bottom
        // contact or a top contact
        ...
        if (print_z < min_print_z + support_layer_height_min) {
            ...
            // With raft active, any contact layer below the raft_contact_top_z will be brought to raft_contact_top_z
            // to extend the raft area.
            print_z = min_print_z;
            ...
            // Don't know the height of the top contact layer yet. The top contact layer is printed with a normal flow
            // and its height will be set adaptively later on.
        }
...
static inline void fill_contact_layer(SupportGeneratorLayer &new_layer,
    ...
    auto lower_layer_polygons_for_dense_interface = [&lower_layer_polygons_for_dense_interface_cache,
        ...
        if (lower_layer_polygons_for_dense_interface_cache.empty())
            lower_layer_polygons_for_dense_interface_cache =
                // FIXME no_interface_offset * 0.6f offset is not quite correct, one shall derive it based on an angle
                // thus depending on layer height.
    ...
    // 1) Contact polygons will be projected down. To keep the interface and base layers from growing, return a
    // contour a tiny bit smaller than the grid cells.
    ...
    if (reduce_interfaces) {
        // Reduce the amount of dense interfaces: Do not generate dense interfaces below overhangs with 60% overhang
        // of the extrusions.
        ...
        if (!dense_interface_polygons.empty()) {
            ...
            // FIXME The regularization of dense_interface_polygons above may stretch dense_interface_polygons outside
            // of the contact polygons,
            // thus some dense interface areas may not get supported. Trim the excess with contact_polygons at the
            // following line. See for example GH #4874.
...
#ifdef SLIC3R_DEBUG
            SVG::export_expolygons(debug_out_path("support-top-contacts-final1-run%d-layer%d-z%f.svg", iRun, layer_id,
                                                  layer.print_z),
                                   {{{union_ex(lower_layer_polygons)}, {"lower_layer_polygons", "gray", 0.2f}},
                                    {{union_ex(*new_layer.contact_polygons)},
                                     {"new_layer.contact_polygons", "yellow", 0.5f}},
                                    {{union_ex(slices_margin.polygons)}, {"slices_margin_cached", "blue", 0.5f}},
                                    {{union_ex(dense_interface_polygons)},
                                     {"dense_interface_polygons", "green", 0.5f}},
                                    {{union_safety_offset_ex(new_layer.polygons)},
                                     {"new_layer.polygons", "red", "black", "", scaled<coord_t>(0.1f), 0.5f}}});
            // support_grid_pattern.serialize(debug_out_path("support-top-contacts-final-run%d-layer%d-z%f.bin", iRun,
            // layer_id, layer.print_z));
            SVG::export_expolygons(debug_out_path("support-top-contacts-final2-run%d-layer%d-z%f.svg", iRun, layer_id,
                                                  layer.print_z),
                                   {{{union_ex(lower_layer_polygons)}, {"lower_layer_polygons", "gray", 0.2f}},
                                    {{union_ex(*new_layer.contact_polygons)},
                                     {"new_layer.contact_polygons", "yellow", 0.5f}},
...
#ifdef SLIC3R_DEBUG
                                                                  ,
                                                                  "top_contact_polygons3", iRun, layer_id,
                                                                  layer.print_z
#endif // SLIC3R_DEBUG
        );
        ensure_valid(new_layer.polygons, support_params.resolution);
    ...
    if (!enforcer_polygons.empty() && !slices_margin.all_polygons.empty() && layer_id > 0) {
        // Support enforcers used together with support enforcers. The support enforcers need to be handled separately
        // from the rest of the support.
        ...
        if (reduce_interfaces) {
            // 2) infill polygons, expand them by half the extrusion width + a tiny bit of extra.
            // Reduce the amount of dense interfaces: Do not generate dense interfaces below overhangs with 60%
            // overhang of the extrusions.
            ...
            if (!dense_interface_polygons.empty()) {
                ...
                // Support islands, to be stretched into a grid.
                // FIXME The regularization of dense_interface_polygons above may stretch dense_interface_polygons
                // outside of the contact polygons,
                // thus some dense interface areas may not get supported. Trim the excess with contact_polygons at the
                // following line. See for example GH #4874.
                Polygons dense_interface_polygons_trimmed = intersection(dense_interface_polygons,
                                                                         *new_layer.enforcer_polygons);
                SupportGridPattern support_grid_pattern(&dense_interface_polygons_trimmed,
...
#ifdef SLIC3R_DEBUG
                                                                    ,
                                                                    "top_contact_polygons5", iRun, layer_id,
                                                                    layer.print_z
#endif // SLIC3R_DEBUG
                );
            }
    ...
    // Even after the contact layer was expanded into a grid, some of the contact islands may be too tiny to be
    // extruded. Remove those tiny islands from new_layer.polygons and new_layer.contact_polygons.
...
// Merge close contact layers conservatively: If two layers are closer than the minimum allowed print layer height
// (the min_layer_height parameter), the top contact layer is merged into the bottom contact layer.
static void merge_contact_layers(const SlicingParameters &slicing_params,
...
// For a soluble interface material synchronize the layer heights with the object, otherwise leave the layer height
// undefined. If supports over bed surface only are requested, don't generate contact layers over an object.
...
#define SLIC3R_IRUN , iRun
    ...
    // So layer_id == 0 means first object layer and layer->id == 0 means first print layer if there are no explicit
    // raft layers.
    size_t num_layers = this->has_support() ? object.layer_count() : 1;
    // For each overhang layer, two supporting layers may be generated: One for the overhangs extruded with a bridging
    // flow, and the other for the overhangs extruded with a normal flow.
    contact_out.assign(num_layers * 2, nullptr);
    Slic3r::parallel_for(size_t(this->has_raft() ? 0 : 1), num_layers,
                         [this, &object, &annotations, &layer_storage, &contact_out](size_t layer_id) {
                             {
                                 const Layer &layer = *object.layers()[layer_id];
                                 Polygons lower_layer_polygons = (layer_id == 0) ?
                                     Polygons() :
                                 ...
                                 auto [overhang_polygons, contact_polygons, enforcer_polygons,
                                       no_interface_offset] = detect_overhangs(layer, layer_id, lower_layer_polygons,
                                                                               *m_print_config, *m_object_config,
                                                                               annotations, slices_margin,
                                 ...
                                 if (!contact_polygons.empty() || !overhang_polygons.empty()) {
                                     ...
                                     auto [new_layer, bridging_layer] =
                                         new_contact_layer(*m_print_config, *m_object_config, *m_slicing_params,
                                                           m_support_params.support_layer_height_min, layer,
                                                           layer_storage);
                                     if (new_layer) {
                                         ...
                                         fill_contact_layer(*new_layer, layer_id, *m_slicing_params, m_support_params,
                                                            *m_object_config, slices_margin, overhang_polygons,
                                                            contact_polygons, enforcer_polygons, lower_layer_polygons,
                                                            no_interface_offset
#ifdef SLIC3R_DEBUG
                                                            ,
                                                            iRun, layer
                                         ...
                                         assert_valid(new_layer->polygons);
                                         // Insert new layer even if there is no interface generated: Likely the
                                         // support angle is not steep enough to require dense interface, however
                                         // generating a sparse support will be useful for the object stability. if (!
                                         ...
                                         if (bridging_layer != nullptr) {
                                             ...
                                             bridging_layer->contact_polygons = std::make_unique<Polygons>(
                                                 *new_layer->contact_polygons);
    ...
    for (auto &layer : contact_out)
        ensure_valid(layer->polygons, m_support_params.resolution);
...
static inline SupportGeneratorLayer *detect_bottom_contacts(
    ...
    const PrintObject &object,
    const Layer &layer,
    // Existing top contact layers, to which this newly created bottom contact layer will be snapped to guarantee a
    ...
    layer_new.height_block = ((object.config().support_material_contact_distance_type.value == zdPlane) ?
    ...
    // FIXME how much to inflate the bottom surface, as it is being extruded with a bridging flow? The following line
    // uses a normal flow.
    layer_new.polygons = expand(touching, double(support_params.support_material_flow.scaled_width()),
                                SUPPORT_SURFACES_OFFSET_PARAMETERS);
    ...
    if (!slicing_params.soluble_interface) {
        ...
        for (size_t top_idx = size_t(std::max<int>(0, contact_idx)); top_idx < top_contacts.size() &&
            ...
            if (top_contacts[top_idx]->print_z >
                layer_new.print_z - support_params.support_layer_height_min - EPSILON) {
                ...
                if (diff > 0.) {
                    // The top contact layer is below this layer. Make the bridging layer thinner to align with the
                    // existing top layer.
                    ...
                    // By making the layer thicker, one will decrease the number of discrete layers with the price of
                    // extruding a bit too thick bridges. By making the layer thinner, one adds one more discrete layer.
                    layer_new.print_z = top_contacts[top_idx]->print_z;
                    layer_new.height -= diff;
                }
                break;
...
#ifdef SLIC3R_DEBUG
            SVG::export_expolygons(debug_out_path("support-support-areas-raw-before-trimming-%d-with-%f-%lf.svg",
...
// Called twice: First for normal supports, possibly trimmed by "on build plate only", second for support enforcers
// not trimmed by "on build plate only".
static inline std::pair<Polygons, Polygons> project_support_to_grid(const Layer &layer,
    ...
    Polygons trimming = layer_buildplate_covered ? std::move(*layer_buildplate_covered) :
                                                   offset(layer.lslices(), double(SCALED_EPSILON));
...
#ifdef SLIC3R_DEBUG
        Slic3r::SVG::export_expolygons(debug_out_path("support-layer_support_area-gridded-%s-%d-%lf.svg", debug_name,
                                                      iRun, layer.print_z),
                                       union_ex(out.first));
#endif /* SLIC3R_DEBUG */
    });

    // 2) Support polygons will be projected down. To keep the interface and base layers from growing, return a
    // contour a tiny bit smaller than the grid cells.
    task_group_inner.run([&grid_params, &support_grid_pattern, &out
#ifdef SLIC3R_DEBUG
                          ,
...
SupportGeneratorLayersPtr PrintObjectSupportMaterial::bottom_contact_layers_and_layer_support_areas(
    ...
    for (int layer_id = int(object.total_layer_count()) - 2; layer_id >= 0; --layer_id) {
       ...
       // Use a slight positive offset to overlap the touching regions.
...
#else
            // Consume the contact_polygons. The contact polygons are already expanded into a grid form, and they are
            // a tiny bit smaller than the grid cells.
...
#endif
            // These are the overhang surfaces. They are touching the object and they are not expanded away from the
            // object. Use a slight positive offset to overlap the touching regions.
        ...
        // Find the bottom contact layers above the top surfaces of this layer.
        task_group.run([this, &object, &layer, &top_contacts, contact_idx, &layer_storage, &layer_support_areas,
            ...
            // Find the bottom contact layers above the top surfaces of this layer.
            SupportGeneratorLayer *layer_new = detect_bottom_contacts(*m_slicing_params, m_support_params, object,
        ...
        //        float column_propagation_filtering_radius = scaled<float>(0.8 * 0.5 *
        //        (m_support_params.support_material_flow.spacing() + m_support_params.support_material_flow.width()));
        task_group.run([&grid_params, &overhangs_projection, &overhangs_projection_raw, &layer, &layer_support_area,
            ...
            // When propagating support areas downwards, stop propagating the support column if it becomes too thin to
            // be printable.
        ...
        if (!enforcers_projection.empty())
            ...
            task_group.run([&grid_params, &enforcers_projection, &enforcers_projection_raw, &layer,
                            &layer_support_area_enforcers
#ifdef SLIC3R_DEBUG
                            ,
                            iRun, layer_id
#endif /* SLIC3R_DEBUG */
                ...
                std::tie(layer_support_area_enforcers,
                         enforcers_projection) = project_support_to_grid(layer, grid_params, enforcers_projection_raw,
                                                                         nullptr
#ifdef SLIC3R_DEBUG
                                                                         ,
...
// Trim the top_contacts layers with the bottom_contacts layers if they overlap, so there would not be enough vertical
...
void PrintObjectSupportMaterial::trim_top_contacts_by_bottom_contacts(const PrintObject &object,
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, int(top_contacts.size())),
                      [&bottom_contacts, &top_contacts](const tbb::blocked_range<int> &range) {
                          ...
                          // For all top contact layers, counting downwards due to the way idx_higher_or_equal caches
                          // the last index to avoid repeated binary search.
                          for (int idx_top = range.end() - 1; idx_top >= range.begin(); --idx_top) {
                              ...
                              idx_bottom_overlapping_first =
                                  idx_lower_or_equal(bottom_contacts, idx_bottom_overlapping_first,
                                                     [&layer_top](const SupportGeneratorLayer *layer_bottom) {
                                                         return layer_bottom->bottom_print_z() - EPSILON <=
                                                             layer_top.bottom_z;
                                                     });
...
SupportGeneratorLayersPtr PrintObjectSupportMaterial::raft_and_intermediate_support_layers(
    ...
    auto layer_extreme_lower = [](const SupportGeneratorLayer *l1, const SupportGeneratorLayer *l2) {
        ...
        return z1 < z2 ||
            (z1 == z2 && l1->layer_type == SupporLayerType::TopContact &&
    ...
    coordf_t support_layer_height = m_object_config->support_material_layer_height.value == 0 ?
        ...
        std::min(m_slicing_params->max_suport_layer_height,
                 std::max(0., // m_slicing_params->min_suport_layer_height,
                          m_object_config->support_material_layer_height.get_abs_value(
                              m_support_params.support_material_flow.nozzle_diameter())));
    coordf_t support_interface_layer_height = m_object_config->support_material_interface_layer_height.value == 0 ?
        m_slicing_params->max_suport_layer_height :
        std::min(m_slicing_params->max_suport_layer_height,
                 std::max(0., // m_slicing_params->min_suport_layer_height,
                          m_object_config->support_material_interface_layer_height.get_abs_value(
    ...
    for (size_t idx_extreme = idx_extreme_first; idx_extreme < extremes.size(); ++idx_extreme) {
        ...
        if (std::abs(extr2z - m_slicing_params->first_print_layer_height) < EPSILON) {
            // This is a bottom of a synchronized (or soluble) top contact layer, its height has been decided in
            // this->top_contact_layers().
        ...
        if (std::abs(extr1z) < EPSILON) {
            // This layer interval starts with the 1st layer. Print the 1st layer using the prescribed 1st layer
            // thickness. assert(! m_slicing_params->has_raft()); RaftingEdition: unclear where the issue is: assert
            // fails with 1-layer raft & base supports
            assert(intermediate_layers.empty() ||
                   intermediate_layers.back()->print_z <= m_slicing_params->first_print_layer_height);
            // At this point only layers above first_print_layer_heigth + EPSILON are expected as the other cases were
            // captured earlier.
            assert(extr2z >= m_slicing_params->first_print_layer_height + EPSILON);
        ...
        if (synchronize) {
            ...
            auto compute_step = []() {};
            {
                ...
                n_layers_bot = m_object_config->support_material_bottom_interface_layers.is_enabled() ?
                    ...
                    n_layers_top;
                n_layers_bot = std::max(size_t(0),
                                        n_layers_bot - 1); // the first bot layer is already in the extreme list.
                ...
                if (dist <= height_top_interface + height_bot_interface - EPSILON) {
                    ...
                    n_layers_middle = size_t(
                        ceil((dist - height_top_interface - height_bot_interface) / support_layer_height));
                    n_layers_total = n_layers_bot + n_layers_middle + n_layers_top;
                    // compute the avg
                    ...
                    if (step < support_layer_height && step < support_interface_layer_height) {
                        ...
                        // The average is between the two, so use the height of the feature that have the lower height.
                        // And use th rest for the other feature It will have somethign between its max and the other one.
                        if (support_interface_layer_height <= support_layer_height) {
                            step_interface = support_interface_layer_height;
                            step = (dist - step_interface * (n_layers_top + n_layers_bot)) /
                                coordf_t(n_layers_middle);
                        } else {
                ...
                // recompute the steps
                {
                    ...
                    if (dist <= height_top_interface + height_bot_interface) {
                        ...
                        if (step < support_layer_height && step < support_interface_layer_height) {
                            ...
                            // The average is between the two, so use the height of the feature that have the lower
                            // height. And use th rest for the other feature It will have somethign between its max
                            // and the other one.
                            if (support_interface_layer_height <= support_layer_height) {
                                step_interface = support_interface_layer_height;
                                step = (dist - height_top_interface - height_bot_interface) /
                                    coordf_t(n_layers_middle);
                            } else {
                                step = support_layer_height;
                                step_interface = (height_top_interface + height_bot_interface) /
                                    coordf_t(n_layers_top + n_layers_bot);
                            }
                        }
                    }
                }
            ...
            for (size_t i = 0; i < n_layers_total; ++i) {
                ...
                if (i + 1 == n_layers_total) {
                    ...
                    if (i < n_layers_bot || i >= n_layers_total - n_layers_top) {
                        ...
                        // ensure the height is multuiple of z_step, and don't go higher than
                        // support_interface_layer_height
                        // taht work because support_interface_layer_height is a multiple of z_step, so it's not
                        // possible to accumulate a big wanted_z
                        layer_new.height = std::min(check_z_step(wanted_z - last_z, this->m_slicing_params->z_step),
                                                    support_interface_layer_height);
                    } else {
                        ...
                        // taht work because support_layer_height is a multiple of z_step, so it's not possible to
                        // accumulate a big wanted_z
                        layer_new.height = std::min(check_z_step(wanted_z - last_z, this->m_slicing_params->z_step),
...
// At this stage there shall be intermediate_layers allocated between bottom_contacts and top_contacts, but they have
// no polygons assigned. Also the bottom/top_contacts shall have a layer thickness assigned already.
void PrintObjectSupportMaterial::generate_base_layers(const PrintObject &object,
    ...
    for (auto layer : intermediate_layers)
        for (Polygon &poly : layer->polygons)
            ;
    ...
    tbb::parallel_for(
        ...
        [this, &object, &bottom_contacts, &top_contacts, &intermediate_layers,
         &layer_support_areas](const tbb::blocked_range<size_t> &range) {
            ...
            // Counting down due to the way idx_lower_or_equal caches indices to avoid repeated binary search over the
            // complete sequence.
            for (int idx_intermediate = int(range.end()) - 1; idx_intermediate >= int(range.begin());
                ...
                assert(idx_intermediate == 0 ||
                       layer_intermediate.print_z + EPSILON >= intermediate_layers[idx_intermediate - 1]->print_z);

                // Find a top_contact layer touching the layer_intermediate from above, if any, and collect its
                // polygons into polygons_new. New polygons for layer_intermediate.
                Polygons polygons_new;

                // Use the precomputed layer_support_areas. "idx_object_layer_above": above means above since the last
                // iteration, not above after this call.
                idx_object_layer_above = idx_lower_or_equal(object.layers().begin(), object.layers().end(),
                                                            idx_object_layer_above,
                                                            [&layer_intermediate](const Layer *layer) {
                                                                return layer->print_z <=
                                                                    layer_intermediate.print_z + EPSILON;
                                                            });
                ...
                // 3) base.print_z > top.print_z  && base.bottom_z >= top.bottom_z -> Overlap, which will be solved
                // inside generate_support_toolpaths() by reducing the base layer height where it overlaps the top
                // layer. No trimming needed here. 4) base.print_z > top.bottom_z && base.bottom_z < top.bottom_z ->
                // Base overlaps with top.bottom_z. This must not happen. 5) base.print_z <= top.print_z  &&
                ...
                if (idx_object_layer_above < 0) {
                    // layer_support_areas are synchronized with object layers and they contain projections of the
                    // contact layers above them. This intermediate layer is not above any object layer, thus there is
                    // no information in layer_support_areas about towers supporting contact layers intersecting the
                    // first object layer. Project these contact layers now.
                ...
                // 3) base.print_z > bottom.bottom_z && base.bottom_z < bottom.bottom_z -> Overlap, which will be
                // solved inside generate_support_toolpaths() by reducing the bottom layer height where it overlaps
                // the base layer. No trimming needed here. 4) base.print_z > bottom.print_z  && base.bottom_z >=
                // bottom.print_z -> Base overlaps with bottom.print_z. This must not happen. 5) base.print_z <=
                // bottom.print_z && base.bottom_z >= bottom.bottom_z -> Base is fully inside top. Trim base by top.
                idx_bottom_contact_overlapping = idx_lower_or_equal(bottom_contacts, idx_bottom_contact_overlapping,
                                                                    [&layer_intermediate](
                                                                        const SupportGeneratorLayer *layer) {
                                                                        return layer->bottom_print_z() <=
                                                                            layer_intermediate.print_z - EPSILON;
                                                                    });
                      ...
                      [this, &object, &nonempty_layers, gap_extra_above, gap_extra_below,
                       gap_xy_scaled](const tbb::blocked_range<size_t> &range) {
                          ...
                          for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer) {
                              ...
                              // BOOST_LOG_TRIVIAL(trace) << "Support generator - trim_support_layers_by_object -
                              // trimmming non-empty layer " << idx_layer << " of " << nonempty_layers.size();
                              assert(!support_layer.polygons.empty() &&
                                     support_layer.print_z >= m_slicing_params->raft_contact_top_z + EPSILON);
                              // Find the overlapping object layers including the extra above / below gap.
                              coordf_t z_threshold = support_layer.print_z - support_layer.height_block -
                                  gap_extra_below + EPSILON;
                              idx_object_layer_overlapping = idx_higher_or_equal(object.layers().begin(),
                                                                                 object.layers().end(),
                              ...
                              for (; i < object.layers().size(); ++i) {
                                  ...
                                  polygons_append(polygons_trimming,
                                                  offset(object_layer.lslices(), gap_xy_scaled,
                                                         SUPPORT_SURFACES_OFFSET_PARAMETERS));
                              }
                              if (!m_slicing_params->soluble_interface) {
                                  ...
                                  for (; i < object.layers().size(); ++i) {
                                      ...
                                      for (LayerRegion *region : object_layer.regions()) {
                                          ...
                                          polygons_append(polygons_trimming,
                                                          offset(to_polygons(region->fill_surfaces().filter_by_type(
                                          ...
                                          if (region->region().config().overhangs_width.value > 0)
                                              ...
                                              SupportMaterialInternal::collect_bridging_perimeter_areas(
                                                  region->perimeters().entities(), gap_xy_scaled, polygons_trimming);
                                      }
