...
void remove_bridges_from_contacts(const PrintConfig &print_config,
    ...
    ExPolygons bridges;
    {
        coordf_t nozzle_diameter = scale_t(
            print_config.nozzle_diameter.get_at(layerm.region().config().perimeter_extruder - 1));
        // Surface supporting this layer, expanded by 0.5 * nozzle_diameter, as we consider this kind of overhang to
        // be sufficiently supported.
        Polygons lower_grown_slices = expand(
            lower_layer.lslices(),
            // FIXME to mimic the decision in the perimeter generator, we should use half the external perimeter width.
            0.5f * float(nozzle_diameter), SUPPORT_SURFACES_OFFSET_PARAMETERS);
...
#else
        Polylines overhang_perimeters = diff_pl(to_polylines(layerm.perimeters().as_polylines(), nozzle_diameter * 2),
                                                lower_grown_slices);
#endif
        ...
        // FIXME one may want to use a maximum of bridging flow width and normal flow width, as the perimeters are
        // calculated using the normal flow
        //  and then turned to bridging flow, thus their centerlines are derived from non-bridging flow and expanding
        //  them by a bridging flow may not expand them to the edge of their respective islands.
...
std::pair<SupportGeneratorLayersPtr, SupportGeneratorLayersPtr> generate_interface_layers(
    ...
    if (!intermediate_layers.empty() && support_params.has_interfaces()) {
        // For all intermediate layers, collect top contact surfaces, which are not further than
        ...
        auto insert_layer =
            [&layer_storage, smooth_supports, closing_distance, smoothing_distance, minimum_island_radius,
             &support_params](SupportGeneratorLayer &intermediate_layer, Polygons &bottom, Polygons &&top,
            ...
            // Merge top / bottom interfaces. For snug supports, merge using closing distance and regularize (close
            // concave corners).
            ...
            if (!bottom.empty()) {
                ...
                if (!bottom.empty()) {
                    ...
                    if (subtract) {
                        ...
                        layer_new.polygons = ensure_valid(support_params.resolution,
                                                          diff(std::move(layer_new.polygons), *subtract));
        ...
        Slic3r::parallel_for(
            size_t(0), intermediate_layers.size(),
            [&bottom_contacts, &top_contacts, &top_interface_layers, &top_base_interface_layers, &intermediate_layers,
             ...
             &base_interface_layers](size_t idx_intermediate_layer) {
                // Gather the top / bottom contact layers intersecting with num_interface_layers resp.
                // num_interface_layers_only intermediate layers above / below this intermediate layer. Index of the
                // first top contact layer intersecting the current intermediate layer.
                ...
                auto num_intermediate = int(intermediate_layers.size());
                {
                    ...
                    if (support_params.num_top_interface_layers > 0) {
                        ...
                        coordf_t top_z = intermediate_layers[std::min(num_intermediate - 1,
                                                                      int(idx_intermediate_layer +
                                                                          support_params.num_top_interface_layers) -
                                                                          1)]
                                             ->print_z;
                        ...
                        if (support_params.num_top_base_interface_layers > 0)
                            ...
                            top_inteface_z = support_params.num_top_interface_layers_only() == 0 ?
                                ...
                                intermediate_layers[std::min(num_intermediate - 1,
                                                             int(idx_intermediate_layer +
                                                                 support_params.num_top_interface_layers_only()) -
                                                                 1)]
                                    ->print_z;
                        ...
                        for (int idx_top_contact = idx_top_contact_first; idx_top_contact < int(top_contacts.size());
                            ...
                            polygons_append(
                                ...
                                // For snug supports, project the overhang polygons covering the whole overhang, so that
                                // they will merge without a gap with support polygons of the other layers. For grid
                                // supports, merging of support regions will be performed by the projection into grid.
                    ...
                    if (support_params.num_bottom_interface_layers > 0) {
                        ...
                        coordf_t bottom_z =
                            intermediate_layers[std::max(0,
                                                         int(idx_intermediate_layer -
                                                             support_params.num_bottom_interface_layers) +
                                                             1)]
                                ->bottom_z;
                    ...
                    if (!polygons_bottom_contact_projected_interface.empty() ||
                        ...
                        interface_layer = insert_layer(intermediate_layer,
                                                       polygons_bottom_contact_projected_interface,
                        ...
                        if (interface_layer != nullptr)
                            assert_valid(interface_layers[idx_intermediate_layer]->polygons);
                    }
                    if (!polygons_bottom_contact_projected_base.empty() ||
                        ...
                        if (base_interface_layers[idx_intermediate_layer] != nullptr)
                            assert_valid(base_interface_layers[idx_intermediate_layer]->polygons);
                    }
                }
            });
        ...
        auto merge_remove_empty = [](SupportGeneratorLayersPtr &in1, SupportGeneratorLayersPtr &in2) {
            auto remove_empty = [](SupportGeneratorLayersPtr &vec) {
                vec.erase(std::remove_if(vec.begin(), vec.end(),
                                         [](const SupportGeneratorLayer *ptr) {
        ...
        for (auto &layer : interface_layers)
            if (layer)
                assert_valid(layer->polygons);
        for (auto &layer : base_interface_layers)
            if (layer)
                assert_valid(layer->polygons);
...
SupportGeneratorLayersPtr generate_raft_base(const PrintObject &object,
                                             ...
                                             SupportGeneratorLayerStorage &layer_storage) {
    for (SupportGeneratorLayersPtr layer_ptr : {top_contacts, interface_layers, base_interface_layers, base_layers}) {
    ...
    if (object.has_brim()) {
        ...
        const bool brim_outer = object.config().brim_width >
            0; // brim_type == btOuterOnly || brim_type == btOuterAndInner;
        const bool brim_inner = object.config().brim_width_interior >
            0; // brim_type == btInnerOnly || brim_type == btOuterAndInner;
        ...
        for (const ExPolygon &ex : object.layers().front()->lslices()) {
            ...
            if (brim_outer && brim_inner)
                ...
                if (brim_outer)
                    polygons_append(brim,
                                    offset(ex.contour, brim_separation, ClipperLib::jtRound, float(scale_(0.1))));
                else
    ...
    // How much to inflate the support columns to be stable. This also applies to the 1st layer, if no raft layers are
    // to be printed.
    ...
    if (base_interfaces != nullptr &&
        base_interfaces->bottom_print_z() > slicing_params.raft_interface_top_z + EPSILON)
        // This is not the raft column base layer.
    ...
    if (slicing_params.raft_layers() > 1) {
        ...
        if (!interface_polygons.empty()) {
            // Merge the untrimmed columns base with the expanded raft interface, to be used for the support base and
            // interface.
        ...
        if (columns_base != nullptr) {
            ...
            if (inflate_factor_1st_layer > SCALED_EPSILON) {
                ...
                auto nsteps = std::max(5,
                                       int(ceil(inflate_factor_1st_layer /
            ...
            if (!interface_polygons.empty())
                columns_base->polygons = ensure_valid(diff(columns_base->polygons, interface_polygons),
                                                      support_params.resolution);
            assert_valid(raft);
...
#ifdef _DEBUGINFO
    for (auto entity : dst)
        entity->visit(LoopAssertVisitor());
...
static inline void tree_supports_generate_paths(ExtrusionEntityCollection &dst,
                                                const Polygons &polygons,
                                                const Flow &flow,
                                                const SupportParameters &support_params) {
#ifdef _DEBUGINFO
    for (const auto extr : dst.entities())
        extr->visit(LoopAssertVisitor());
    ...
    auto shrink_expolygon_with_contour_idx = [](const Slic3r::ExPolygon &expoly, const float delta,
                                                ClipperLib::JoinType joinType, double miterLimit,
                                                ClipperLib_Z::Paths &out) -> int {
        assert(delta > 0);
        ...
        if (expoly.holes.empty()) {
            ...
            if (holes.empty()) {
                ...
                clipper.Execute(ClipperLib_Z::ctDifference, output, ClipperLib_Z::pftNonZero,
    ...
    ExPolygons expolys = closing_ex(polygons, float(SCALED_EPSILON),
    ...
    for (ExPolygon &expoly : expolys) {
            ...
            if (double area = expoly.area(); area > support_params.tree_branch_diameter_double_wall_area_scaled) {
                ...
                if (level2.size() == 1) {
                    extrusion_entities_append_paths(
                        *eec, ensure_valid(draw_perimeters(expoly, clip_length), support_params.resolution),
        ...
        for (int idx_loop = 0; idx_loop < int(expoly.num_contours()); ++idx_loop) {
            ...
            // Orient all contours CW, because the anchor will be added to the end of polyline while we want to start
            // a loop with the anchor.
            if (idx_loop == 0)
            ...
            if (pl.size() < 2)
                continue;
            // Find the foot of the seam point on anchor_candidates. Only pick an anchor point that was created by
            // offsetting the source contour.
            ...
            for (ClipperLib_Z::Path &path : anchor_candidates)
                for (int i = 0; i < int(path.size()); ++i) {
                    ...
                    if (path[i].z() == idx_loop || path[j].z() == idx_loop) {
                        ...
                        if ((path[i].z() == idx_loop || t > EPSILON) &&
                            (path[j].z() == idx_loop || t < 1. - EPSILON)) {
                            // Closest point.
                            Vec2d fp = pi + v * t;
...
//
//    if (! with_sheath) {
//        fill_expolygons_generate_paths(dst, closing_ex(polygons, float(SCALED_EPSILON)), filler, density, role,
//        flow); return;
//    }
//
//    FillParams fill_params;
...
//    const double clip_length = spacing * 0.15;
//
//    for (ExPolygon &expoly : closing_ex(polygons, float(SCALED_EPSILON), float(SCALED_EPSILON +
...
//        extrusion_entities_append_paths(out, draw_perimeters(expoly, clip_length), { ExtrusionRole::SupportMaterial,
//        flow });
//        // Fill in the rest.
//        fill_expolygons_generate_paths(out, offset_ex(expoly, float(-0.4 * spacing)), filler, fill_params, density,
//        role, flow); if (no_sort && ! eec->empty())
...
struct SupportGeneratorLayerExtruded
{
    ...
    void merge(SupportGeneratorLayerExtruded &&other, coord_t resolution) {
        ...
        if (other.m_polygons_to_extrude != nullptr) {
            if (m_polygons_to_extrude == nullptr) {
                // This layer has no extrusions generated yet, if it has no m_polygons_to_extrude (its area to extrude
                // was not reduced yet).
            ...
            *m_polygons_to_extrude = union_safety_offset(*m_polygons_to_extrude);
            ...
            // The other layer has no extrusions generated yet, if it has no m_polygons_to_extrude (its area to
            // extrude was not reduced yet).
...
private:
    // In case the extrusions are non-empty, m_polygons_to_extrude may contain the rest areas yet to be filled by additional
    // support. This is useful mainly for the loop interfaces, which are generated before the zig-zag infills.
...
struct LoopInterfaceProcessor
{
    ...
    void generate(SupportGeneratorLayerExtruded &top_contact_layer,
                  const Flow &interface_flow_src,
...
void LoopInterfaceProcessor::generate(SupportGeneratorLayerExtruded &top_contact_layer,
        ...
        for (ExPolygons::iterator it_contact_expoly = top_contact_expolygons.begin();
            ...
            for (size_t i_contour = 0; i_contour <= it_contact_expoly->holes.size(); ++i_contour) {
                ...
                if (!intersection_pl(contour.split_at_first_point(), overhang_with_margin).empty()) {
                    ...
                    // Walk around the contour and place circles so their centers are not closer than circle_distance
                    // from each other.
                    ...
                    for (Points::const_iterator it = contour.points.begin() + 1; it != contour.points.end(); ++it) {
                        // Is it possible to place a circle on this segment? Is it not too close to any of the circles
                        // already placed on this contour?
                        const Point &p1 = *(it - 1);
                        const Point &p2 = *it;
                        ...
                        if (disc > 0.) {
                            ...
                            std::pair<const Point *, coordf_t> circle_closest = circle_centers_lookup.find(
                                center_last);
    ...
    Polylines loop_lines;
    {
        ...
        for (int i = 1; i < n_contact_loops; ++i)
            polygons_append(loop_polygons,
                            opening(loops0, i * flow.scaled_spacing() + 0.5f * flow.scaled_spacing(),
        ...
        // Because a closed loop has been split to a line, loop_lines may contain continuous segments split to 2
        // pieces. Try to connect them.
    ...
    top_contact_layer.set_polygons_to_extrude(
        ensure_valid(diff(top_contact_layer.layer->polygons, offset(loop_lines, double(circle_radius * 1.1))),
                     resolution));
...
// When extruding a bottom interface layer over an object, the bottom interface layer is extruded in a thin air,
// therefore it is being extruded with a bridging flow to not shrink excessively (the die swell effect). Tiny
// extrusions are better avoided and it is always better to anchor the thread to an existing support structure if
// possible. Therefore the bottom interface spots are expanded a bit. The expanded regions may overlap with another
// bottom interface layers, leading to over extrusion, where they overlap. The over extrusion is better avoided as it
// often makes the interface layers to stick too firmly to the object.
//
...
static void modulate_extrusion_by_overlapping_layers(
    ...
    for (auto &frag : path_fragments)
        assert(frag.polylines.empty());
    ...
    for (ExtrusionEntitiesPtr::const_iterator it = extrusions_in_out.set_entities().begin();
         it != extrusions_in_out.set_entities().end(); ++it) {
    ...
    for (int i_overlapping_layer = int(n_overlapping_layers) - 1; i_overlapping_layer >= 0; --i_overlapping_layer) {
        ...
        // Adjust the extrusion parameters for a reduced layer height and a non-bridging flow (nozzle_dmr = -1, does
        // not matter).
        ...
        // don't recompute the flow, just use a simple % reduction/increase, as it can wreak havoc as the flow is
        // compute internally by the fill algo. We can't recompute it!
        frag.flow.mm3_per_mm = frag.flow.mm3_per_mm * frag.flow.height /
            old_height; // Flow(frag.flow.width, frag.flow.height, -1.f).mm3_per_mm();
#ifdef SLIC3R_DEBUG
    ...
    for (size_t i_path = 0; i_path < path_ends.size(); ++i_path) {
        ...
        for (;;) {
            ...
            if (path != nullptr) {
                ...
                assert(this_layer.layer_type == SupporLayerType::BottomContact ||
                       path->height() != frag.flow.height || path->mm3_per_mm() != frag.flow.mm3_per_mm);
                if (path->height() != frag.flow.height || path->mm3_per_mm() != frag.flow.mm3_per_mm)
                    path = nullptr;
                // Merging with the previous path. This can only happen if the current layer was reduced by a base
                // layer, which was split into a base and interface layer.
            }
            if (path == nullptr) {
                // Allocate a new path.
            ...
            // assert(frag_polyline.front() == pt_current); // find an exemple where there's a 1 unit diff in y. worth
            // figthing for?
        ...
        if (!multipath.paths.empty()) {
            if (multipath.paths.size() == 1) {
                ...
                // This path was fragmented. Copy the collection as a whole object, so the order inside the collection
                // will not be changed during the chaining of extrusions_in_out.
            ...
            if (this_layer_contacts_only) {
                ...
                for (auto it = object.support_layers().rbegin(); it != object.support_layers().rend(); ++it)
                    if (const SupportLayer &other_layer = **it;
                        ...
                        // other_layer supports this top contact layer. Assign a different support interface direction
                        // to this layer from the layer that supports it.
         ...
         link_max_length_factor](const tbb::blocked_range<size_t> &range) {
            for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++support_layer_id) {
                ...
                std::unique_ptr<Fill> filler_interface = std::unique_ptr<Fill>(Fill::new_from_type(
                    support_params
                        .raft_interface_fill_pattern)); // m_support_params.contact_top_fill_pattern)); FIXME choose
                ...
                std::unique_ptr<FillWithPerimeter> filler_support_with_sheath = std::make_unique<FillWithPerimeter>(
                    (Fill::new_from_type(support_params.base_fill_pattern)));
                filler_support_with_sheath->overlap = support_params.gap_xy == 0 ?
                    0 :
                    1; // don't allow perimater overlap to touch perimeters
                filler_support_with_sheath->ratio_fill_inside = 0.2f;
                std::unique_ptr<FillWithPerimeter> filler_dense = std::make_unique<FillWithPerimeter>(
                    (Fill::new_from_type(ipRectilinear)));
                filler_dense->overlap = support_params.gap_xy == 0 ? 0 : 1;
                filler_dense->ratio_fill_inside = 0.2f;
                filler_interface->set_bounding_box(bbox_object);
                filler_support->set_bounding_box(bbox_object);

                // Print the tree supports cutting through the raft with the exception of the 1st layer, where a full
                // support layer will be printed below both the raft and the trees. Trim the raft layers with the tree
                // polygons.
                const Polygons &tree_polygons = support_layer_id > 0 &&
                        support_layer_id < intermediate_layers.size() &&
                ...
                // Print the support base below the support columns, or the support base for the support columns plus
                // the contacts.
                if (support_layer_id > 0) {
                    ...
                    Flow flow = Flow::new_from_width(float(support_params.raft_flow.width()),
                                                     support_params.raft_flow.nozzle_diameter(),
                                                     float(raft_layer.height),
                                                     support_params.raft_flow.spacing_ratio());
                    assert(!raft_layer.bridging);
                    if (!to_infill_polygons.empty()) {
                        Fill *filler = support_params.with_sheath ? filler_support_with_sheath.get() :
                                                                    filler_support.get();
                        filler->angle = support_params.raft_angle_base;
                        filler->link_max_length = scale_t(support_params.raft_flow.spacing() *
                                                          link_max_length_factor / support_params.support_density);
                        fill_expolygons_generate_paths(
                            // Destination
                            support_layer.support_fills.set_entities(),
                            // Regions to fill
                            closing_ex(tree_polygons.empty() ? to_infill_polygons :
                                                               diff(to_infill_polygons, tree_polygons),
                                       float(SCALED_EPSILON), float(SCALED_EPSILON + 0.5 * flow.scaled_width())),
                            // Filler and its parameters
                            filler, float(support_params.support_density),
                            // Extrusion parameters
                            ExtrusionRole::SupportMaterial, flow, support_params.raft_flow.spacing(),
                            support_params.default_region_config);
#ifndef NDEBUG
...
coordf_t bridge_flow_ratio = 0;
                    ...
                    for (size_t region_id = 0; region_id < support_layer.object()->num_printing_regions();
                         ++region_id) {
                        const PrintRegion &region = support_layer.object()->printing_region(region_id);
                        bridge_flow_ratio += region.config().bridge_flow_ratio.get_abs_value(1.);
                    }

                    // Normalize if needed
                    size_t num_regions = support_layer.object()->num_printing_regions();
                    if (num_regions > 0)
                        bridge_flow_ratio /= num_regions;

                    float width = std::sqrt(bridge_flow_ratio) * nzd;
                    float height = support_layer.height; 
                ...
                fill_expolygons_generate_paths(
                    ...
                    support_layer.support_fills.set_entities(),
                    // Regions to fill
                    closing_ex(tree_polygons.empty() ? raft_layer.polygons : diff(raft_layer.polygons, tree_polygons),
                               float(SCALED_EPSILON), float(SCALED_EPSILON + 0.5 * flow.scaled_width())),
                    // Filler and its parameters
                    filler, density,
                    // Extrusion parameters
    ...
    std::unique_ptr<FillWithPerimeter> filler_first_layer = std::make_unique<FillWithPerimeter>(
        Fill::new_from_type(ipRectilinear));
    ...
    tbb::parallel_for(
        ...
        [&config, &slicing_params, &support_params, &support_layers, &bottom_contacts, &top_contacts,
         ...
         &bbox_object, n_raft_layers, link_max_length_factor, &filler_first_layer,
         raft_top_interface_idx](const tbb::blocked_range<size_t> &range) {
            ...
            std::unique_ptr<Fill> filler_top_interface = std::unique_ptr<Fill>(
                Fill::new_from_type(support_params.contact_top_fill_pattern));
            std::unique_ptr<Fill> filler_bottom_interface = std::unique_ptr<Fill>(
                Fill::new_from_type(support_params.contact_bottom_fill_pattern));
            std::unique_ptr<Fill> filler_intermediate_interface = std::unique_ptr<Fill>(
                Fill::new_from_type(ipRectilinear));
            ...
            // Filler for the base interface (to be used for soluble interface / non soluble base, to produce non
            // soluble interface layer below soluble interface layer).
            std::unique_ptr<Fill> filler_base_interface = base_interface_layers.empty() ?
                std::unique_ptr<Fill>{} :
                std::unique_ptr<Fill>{Fill::new_from_type(
                    support_params.interface_density > 0.95 || support_params.with_sheath ? ipRectilinear :
                                                                                            ipSupportBase)};
            ...
            for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++support_layer_id) {
                ...
                if (support_params.interface_angle_incr == 0 && config.support_material_style.value == smsSnug) {
                    ...
                    interface_angle_delta = support_params.interface_angle +
                        support_layer.interface_id() * support_params.interface_angle_incr;
                }
                ...
                } else if (support_params.base_angle_height > 0 &&
                           (int(support_layer.print_z / support_params.base_angle_height)) % 2 == 1) {
                ...
                if (bottom_contact_layer.layer)
                    assert_valid(bottom_contact_layer.polygons_to_extrude());
                if (top_contact_layer.layer)
                    assert_valid(top_contact_layer.polygons_to_extrude());
                bool has_top_contact_layer = top_contact_layer.layer != nullptr;
                if (base_layer.layer)
                    assert_valid(base_layer.polygons_to_extrude());
                if (interface_layer.layer)
                    assert_valid(interface_layer.polygons_to_extrude());
                if (base_interface_layer.layer)
                    assert_valid(base_interface_layer.polygons_to_extrude());
                ...
                if (config.support_material_interface_layers == 0) {
                    // If no top interface layers were requested, we treat the contact layer exactly as a generic base
                    // layer. Don't merge the raft contact layer though.
                    ...
                    // Merge interface_layer into top_contact_layer, as the top_contact_layer is not synchronized and
                    // therefore it will be used to trim other layers.
            ...
            if ( ! interface_layer.empty() && ! base_layer.empty()) {
                ...
                auto extrude_interface = [&](SupportGeneratorLayerExtruded &layer_ex,
                                             InterfaceLayerType interface_layer_type) {
#ifdef _DEBUGINFO
                    for (const auto extr : layer_ex.extrusions.entities())
                        extr->visit(LoopAssertVisitor());
#endif
                    if (!layer_ex.empty() && !layer_ex.polygons_to_extrude().empty()) {
                        ...
                        // FIXME Bottom interfaces are extruded with the briding flow. Some bridging layers have its
                        // height slightly reduced, therefore
                        //  the bridging flow does not quite apply. Reduce the flow to area of an ellipse? (A = pi * a * b)
                        Fill *filler = interface_layer_type == InterfaceLayerType::TopContact ?
                            filler_top_interface.get() :
                            interface_layer_type == InterfaceLayerType::BottomContact ?
                            filler_bottom_interface.get() :
                        ...
                        float width = std::sqrt(bridge_flow_ratio) * support_params.support_material_bottom_interface_flow.nozzle_diameter();
                        ...
                        // filler->layer_id = support_layer_id; // don't do that, or the filler will rotate thigns
                        // from that layerid
                        ...
                        if (support_layer.id() == 0 && layer_ex.layer->bottom_z <= 0) {
                            ...
                            if (raft_top_interface_idx == support_layer_id) {
                                ...
                                supp_density = interface_as_base ? support_params.support_density :
                                                                   support_params.interface_density;
                                ...
                                supp_density = interface_as_base ? support_params.support_density :
                                                                   support_params.interface_density;
                                filler_spacing = interface_as_base ?
                        ...
                        // filler->angle = interface_as_base ?
                        //         // If zero interface layers are configured, use the same angle as for the base
                        //         layers. angles[support_layer_id % angles.size()] :
                        //         // Use interface angle for the interface layers.
                        //         raft_contact ?
                        //             support_params.raft_interface_angle(support_layer.interface_id()) :
                        //             support_interface_angle;
                        // double density = raft_contact ? support_params.raft_interface_density : interface_as_base ?
                        // support_params.support_density : support_params.interface_density; filler->spacing =
                        ...
                        fill_expolygons_generate_paths(
                            ...
                            interface_as_base ? ExtrusionRole::SupportMaterial :
                                                ExtrusionRole::SupportMaterialInterface,
                            interface_flow, filler_spacing, support_params.default_region_config);
                    }
                };
                ...
                const bool bottom_interfaces = config.support_material_bottom_interface_layers.is_enabled() ?
                    ...
                    top_interfaces;
                extrude_interface(top_contact_layer,
                                  raft_layer         ? InterfaceLayerType::RaftContact :
                                      top_interfaces ? InterfaceLayerType::TopContact :
                                                       InterfaceLayerType::InterfaceAsBase);
                extrude_interface(bottom_contact_layer,
                                  bottom_interfaces ? InterfaceLayerType::BottomContact :
                                                      InterfaceLayerType::InterfaceAsBase);
                extrude_interface(interface_layer,
                                  top_interfaces ? InterfaceLayerType::Interface :
                                                   InterfaceLayerType::InterfaceAsBase);
                ...
                if (!base_interface_layer.empty() && !base_interface_layer.polygons_to_extrude().empty()) {
                    ...
                    // FIXME Bottom interfaces are extruded with the briding flow. Some bridging layers have its height
                    ...
                    filler->link_max_length = scale_t(filler->get_spacing() * link_max_length_factor /
                                                      support_params.interface_density);
                    fill_expolygons_generate_paths(
                        // Destination
                        ...
                        // Extrusion parameters
                        ExtrusionRole::SupportMaterial, interface_flow, filler_spacing,
                        support_params.default_region_config);
                }
                ...
                if (!base_layer.empty() && !base_layer.polygons_to_extrude().empty()) {
                    ...
                    if (base_layer.layer->bottom_z < EPSILON) {
                        ...
                        filler->link_max_length = scale_t(filler_spacing * link_max_length_factor /
                    ...
                    if (!done) {
                        ...
                        fill_expolygons_generate_paths( // TODO: 2.7 test if pattern has the sheath
                            ...
                            filler, density,
                            // Extrusion parameters
                            ExtrusionRole::SupportMaterial, flow, filler_spacing,
                            support_params.default_region_config);
#ifdef _DEBUGINFO
                        for (auto entity : base_layer.extrusions)
                            entity->visit(LoopAssertVisitor());
#endif
                    }
                }
                ...
                if (!base_layer.empty() && !base_interface_layer.empty() &&
                    !base_layer.polygons_to_extrude().empty() &&
                ...
                for (LayerCacheItem &layer_cache_item : layer_cache.nonempty) {
                    ...
                    // in a sense that they are not synchronized with other support layers. As the top and bottom
                    // contact surfaces are inflated to achieve a better anchoring, it may happen, that these surfaces
                    // will at least partially overlap in Z with another support layers, leading to over-extrusion. Mitigate
                    // the over-extrusion by modulating the extrusion rate over these regions. The print head will
                    // follow the same print_z, but the layer thickness will be reduced where it overlaps with another
                    // support layer.
                    // FIXME When printing a briging path, what is an equivalent height of the squished extrudate of
                    // the same width?
                    ...
                    auto add_overlapping = [&layer_cache_item, bottom_z](const SupportGeneratorLayersPtr &layers,
                                                                         size_t idx_top) {
                ...
                if (!polys.empty()) {
                    ...
                    for (const ExPolygon &expoly : support_layer.support_islands)
                        support_layer.support_islands_bboxes.emplace_back(
                            get_extents(expoly).inflated(SCALED_EPSILON));
                }
                          ...
                          for (size_t support_layer_id = range.begin(); support_layer_id < range.end();
                              ...
                              for (LayerCacheItem &layer_cache_item : layer_cache.nonempty) {
                                  if (layer_cache_item.layer_extruded->extrusions.empty())
                                      continue;
#ifdef _DEBUGINFO
                                  for (auto entity : layer_cache_item.layer_extruded->extrusions)
                                      entity->visit(LoopAssertVisitor());
#endif
