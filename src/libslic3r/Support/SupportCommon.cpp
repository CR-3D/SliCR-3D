    ...
    Polygons            &contact_polygons)
{
    ...
    #else
       Polylines overhang_perimeters = diff_pl(layerm.perimeters().as_polyline().to_polyline(), lower_grown_slices);
    #endif
    ...
    const SupportGeneratorLayersPtr      &overlapping_layers)
{
    ...
    for (size_t i_path = 0; i_path < path_ends.size(); ++ i_path) {
        ...
        for (;;) {
           ...
           if (! path->polyline.to_polyline().empty())
                path->polyline.to_polyline().points.pop_back();
            ...
            pt_current = path->polyline.to_polyline().points.back();
...
//    const coordf_t link_max_length_factor = 3.;
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
       ...
       auto filler_interface       = std::unique_ptr<Fill>(Fill::new_from_type(support_params.contact_top_fill_pattern));
        // Filler for the 1st layer interface, if different from filler_interface.
       auto filler_first_layer_ptr = std::unique_ptr<Fill>(range.begin() == 0 && support_params.contact_top_fill_pattern != fill_type_first_layer ? Fill::new_from_type(fill_type_first_layer) : nullptr);
