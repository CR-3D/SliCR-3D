 ...
 * - Generate all to-be-supported points at once instead of sequentially: See branch interlocking_gen
 * PolygonUtils::spreadDots (Or work with sparse grids.)
...
Generator::Generator(const PrintObject &print_object,
    ...
    // Note: There's not going to be a layer below the first one, so the 'initial layer height' doesn't have to be
    // taken into account.
...
void Generator::generateInitialInternalOverhangs(const PrintObject &print_object,
    ...
    // Iterate from top to bottom, to subtract the overhang areas above from the overhang areas on the layer below, to
    // get only overhang in the top layer where it is overhanging.
