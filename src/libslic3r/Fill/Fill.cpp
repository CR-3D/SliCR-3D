///|/ Copyright (c) Prusa Research 2016 - 2023 Lukáš Matěna @lukasmatena, Vojtěch Bubník @bubnikv, Pavel Mikuš
///@Godrak, Lukáš Hejl @hejllukas
...
#define RETURN_COMPARE_NON_EQUAL_TYPED(TYPE, KEY) \
    ...
    if (TYPE(this->KEY) > TYPE(rhs.KEY)) \
        ...
        // Sort first by decreasing bridging angle, so that the bridges are processed with priority when trimming one
        // layer by the other.
...
std::vector<SurfaceFill> group_fills(const Layer &layer) {
    ...
    std::vector<std::vector<const SurfaceFillParams *>>
        region_to_surface_params(layer.regions().size(), std::vector<const SurfaceFillParams *>());
    SurfaceFillParams params;
            ...
            if (surface.surface_type == stInternalVoid)
                ...
                if (surface.is_solid()) {
                    ...
                    params.pattern = (surface.is_external() && !is_bridge) ?
                        (surface.is_top() ? region_config.top_fill_pattern.value :
                                            region_config.bottom_fill_pattern.value) :
                        fill_type_monotonic(region_config.top_fill_pattern) ? ipMonotonic :
                                                                              ipRectilinear;
                ...
                if (surface.is_solid() || is_bridge) {
                    ...
                    // Internal infill. Calculating infill line spacing independent of the current layer height and 1st
                    // layer status, so that internall infill will be aligned over all layers of the current region.
    ...
    if (has_internal_voids) {
        ...
        if (!voids.empty() && !surfaces_polygons.empty()) {
            ...
            // Corners of infill regions, which would not be filled with an extrusion path with a radius of
            ...
            // FIXME why the voids are added to collapsed here? First it is expensive, second the result may lead to
            // some unwanted regions being
            ...
            if (internal_solid_fill == nullptr) {
                ...
                params.pattern = fill_type_monotonic(layerm.region().config().top_fill_pattern) ? ipMonotonic :
                                                                                                  ipRectilinear;
                params.density = 100.f;
                params.extrusion_role = ExtrusionRole::InternalInfill;
    ...
    // Use ipEnsuring pattern for all internal Solids.
    {
        for (size_t surface_fill_id = 0; surface_fill_id < surface_fills.size(); ++surface_fill_id)
            if (SurfaceFill &fill = surface_fills[surface_fill_id];
                fill.surface.surface_type == stInternalSolid || fill.surface.surface_type == stSolidOverBridge) {
...
static void insert_fills_into_islands(Layer &layer, uint32_t fill_region_id, uint32_t fill_begin, uint32_t fill_end) {
    if (fill_begin < fill_end) {
        ...
        // Traverse the slices in an increasing order of bounding box size, so that the islands inside another islands
        // are tested first, so we can just test a point inside ExPolygon::contour and we may skip testing the holes.
        auto point_inside_surface = [&layer](const size_t lslice_idx, const Point &point) {
            ...
            return point.x() >= bbox.min.x() && point.x() < bbox.max.x() && point.y() >= bbox.min.y() &&
                point.y() < bbox.max.y() && layer.lslices[lslice_idx].contour.contains(point);
        };
            ...
            if (lslice.islands.size() == 1) {
                ...
                for (LayerIsland &li : lslice.islands) {
                    ...
                    for (uint32_t fill_expolygon_id : li.fill_expolygons)
                        if (bboxes[fill_expolygon_id].contains(point) &&
                            expolygons[fill_expolygon_id].contains(point)) {
                            island = &li;
                            goto found;
                        }
                }
                // 2) Find closest fill_expolygon, branch and bound by distance to bounding box.
                {
                    ...
                    for (uint32_t island_idx = 0; island_idx < uint32_t(lslice.islands.size()); ++island_idx) {
                        ...
                        const BoundingBoxes &bboxes = li.fill_expolygons_composite() ?
                            ...
                            layer.get_region(li.fill_region_id)->fill_expolygons_bboxes();
                        for (uint32_t fill_expolygon_id : li.fill_expolygons)
                            islands_sorted.push_back({island_idx, fill_expolygon_id,
                                                      bbox_point_distance_squared(bboxes[fill_expolygon_id], point)});
                    }
                    std::sort(islands_sorted.begin(), islands_sorted.end(),
                              [](auto &l, auto &r) { return l.distance2 < r.distance2; });
                    auto dist_min2 = std::numeric_limits<double>::max();
                    for (uint32_t sorted_bbox_idx = 0; sorted_bbox_idx < uint32_t(islands_sorted.size());
                         ++sorted_bbox_idx) {
                        ...
                        const ExPolygons &expolygons = li.fill_expolygons_composite() ?
                            layer.get_region(li.perimeters.region())->fill_expolygons_composite() :
                            layer.get_region(li.fill_region_id)->fill_expolygons();
                        double d2 = (expolygons[isl.expolygon_idx].point_projection(point) - point)
                                        .cast<double>()
                                        .squaredNorm();
                        if (d2 < dist_min2) {
                            dist_min2 = d2;
...
//            printf("flow spacing: %f,  is_external: %d, link_max_length: %lf\n", flow.spacing(), int(surface.is_external()), link_max_length);
        ...
        params.use_arachne = (perimeter_generator == PerimeterGeneratorType::Arachne &&
        ...
        for (ExPolygon &expoly : surface_fill.expolygons) {
            ...
            if (!polylines.empty() || !thick_polylines.empty()) {
                ...
                if (params.use_arachne) {
                    for (const ThickPolyline &thick_polyline : thick_polylines) {
                        ...
                        ExtrusionMultiPath multi_path =
                            PerimeterGenerator::thick_polyline_to_multi_path(thick_polyline,
                    ...
                    // When prefer_clockwise_movements is true, we have to ensure that extrusion paths will not be
                    // reversed during path planning.
                    extrusion_entities_append_paths(eec->entities, std::move(polylines),
                                                    ExtrusionAttributes{surface_fill.params.extrusion_role,
                                                                        ExtrusionFlow{flow_mm3_per_mm,
                                                                                      float(flow_width),
                ...
                insert_fills_into_islands(*this, uint32_t(surface_fill.region_id), fill_begin,
    ...
    for (LayerSlice &lslice : this->lslices_ex)
        for (LayerIsland &island : lslice.islands) {
            if (!island.thin_fills.empty()) {
                ...
                // Fills are always stored as collections, the rest of the pipeline (wipe into infill, G-code
...
void Layer::make_ironing() {
    ...
    // A surface will be ironed with the same extruder to not contaminate the print with another material leaking from
    // the nozzle.
