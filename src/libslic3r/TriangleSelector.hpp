...
private:
    ...
    bool select_triangle_recursive(int facet_idx, const Vec3i32 &neighbors, TriangleStateType type, bool triangle_splitting);
    ...
    void perform_split(int facet_idx, const Vec3i32 &neighbors, TriangleStateType old_state);
    ...
    template<AdditionalMeshInfo facet_info>
    void get_facets_split_by_tjoints(const Vec3i32 &vertices, const Vec3i32 &neighbors, uint8_t color, std::vector<stl_triangle_vertex_indices> &out_triangles, std::vector<uint8_t> &out_colors) const;
    ...
    void bucket_fill_fill_gaps(const std::vector<int>   &gap_fill_candidate_facets, // Facet of the mesh (unsplit), which needs to be checked if the surrounding gap can be filled (selected).
                               ...
                               const std::vector<Vec3i32> &neighbors_propagate);
