...
ExtrusionMultiPath PerimeterGenerator::thick_polyline_to_multi_path(const ThickPolyline &thick_polyline,
    ...
    for (int i = 0; i < (int) lines.size(); ++i) {
        ...
        if (line_len < SCALED_EPSILON) {
            ...
            if (!path.empty())
                path.as_polyline().to_polyline().points.back() =
                    line.b; // If the variable path is non-empty, connect this tiny line to it.
            else if (i + 1 <
                     (int) lines.size()) // If there is at least one following line, connect this tiny line to it.
                lines[i + 1].a = line.a;
            else if (!multi_path.paths.empty())
                multi_path.paths.back().as_polyline().to_polyline().points.back() = line.b; // Connect this tiny line to the last finished path.
        ...
        if (thickness_delta > tolerance) {
            ...
            std::vector<coordf_t> width;
            {
                ...
                for (size_t j = 1; j < segments; ++j) {
                    pp.push_back(
                        (line.a.cast<double>() + (line.b - line.a).cast<double>().normalized() * (j * seg_len))
                            .cast<coord_t>());
        ...
        const Flow new_flow = (role.is_bridge() && flow.bridge()) ?
            flow :
...
static void variable_width_classic(const ThickPolylines &polylines,
                                   ExtrusionRole role,
                                   const Flow &flow,
...
static ExtrusionEntityCollection traverse_loops_classic(const PerimeterGenerator::Parameters &params,
    ...
    for (const PerimeterGeneratorLoop &loop : loops) {
        ...
        ExtrusionRole role_overhang = role_normal | ExtrusionRoleModifier::ERM_Bridge;
        ...
        if (params.config.overhangs_width && params.layer_id > params.object_config.raft_layers &&
            ...
            extrusion_paths_append(paths, intersection_pl({polygon}, lower_slices_polygons_clipped),
            ...
            extrusion_paths_append(paths, diff_pl({polygon}, lower_slices_polygons_clipped),
    ...
    if (!thin_walls.empty()) {
        variable_width_classic(thin_walls, ExtrusionRole::ExternalPerimeter, params.ext_perimeter_flow,
                               coll.m_entities);
        thin_walls.clear();
    }
    ...
    std::vector<std::pair<size_t, bool>> chain = chain_extrusion_entities(coll.m_entities, &zero_point);
    ExtrusionEntityCollection out;
    for (const std::pair<size_t, bool> &idx : chain) {
        assert(coll.m_entities[idx.first] != nullptr);
        if (idx.first >= loops.size()) {
            // This is a thin wall.
            out.m_entities.reserve(out.m_entities.size() + 1);
            out.m_entities.emplace_back(coll.m_entities[idx.first]);
            coll.m_entities[idx.first] = nullptr;
            if (idx.second)
                out.m_entities.back()->reverse();
        } else {
            const PerimeterGeneratorLoop &loop = loops[idx.first];
            assert(thin_walls.empty());
            ExtrusionEntityCollection children = traverse_loops_classic(params, lower_slices_polygons_cache,
                                                                        loop.children, thin_walls);
            out.m_entities.reserve(out.m_entities.size() + children.m_entities.size() + 1);
            ExtrusionLoop *eloop = static_cast<ExtrusionLoop *>(coll.m_entities[idx.first]);
            coll.m_entities[idx.first] = nullptr;
            if (loop.is_contour) {
                if (eloop->is_clockwise())
                    eloop->reverse();
                out.append(std::move(children.m_entities));
                out.m_entities.emplace_back(eloop);
            } else {
...
static ClipperLib_Z::Paths clip_extrusion(const ClipperLib_Z::Path &subject,
    ...
    clipper.ZFillFunction([](const ClipperLib_Z::IntPoint &e1bot, const ClipperLib_Z::IntPoint &e1top,
        ...
        // The clipping function used may produce self intersections outside of the "subject" bounding box. Such self
        // intersections are harmless to the result of the clipping operation, Both ends of each edge belong to the
        // same source: Either they are from subject or from clipping path.
    ...
    for (ClipperLib_Z::Path &path : clipped_paths)
        for (ClipperLib_Z::IntPoint &c_pt : path)
            if (c_pt.z() == 0) {
                ...
                c_pt.z() = coord_t(double(it_min->z()) +
                                   (dist / line_len) * double(std::next(it_min)->z() - it_min->z()));
            }
        ...
        if (params.config.overhangs_width && params.layer_id > params.object_config.raft_layers &&
            ...
            lower_slices_paths.reserve(lower_slices_polygons_cache.size());
            {
                ...
                for (const Polygon &poly : lower_slices_polygons_cache) {
                    clipped.clear();
                    ClipperUtils::clip_clipper_polyline_with_subject_bbox(poly.points,
                                                                         extrusion_path_bbox,
                                                                         clipped);
                    if (!clipped.empty()) {
            ...
            extrusion_paths_append(paths,
                                   clip_extrusion(extrusion_path, lower_slices_paths, ClipperLib_Z::ctIntersection),
                                   role_normal, is_external ? params.ext_perimeter_flow : params.perimeter_flow);

            // get overhang paths by checking what parts of this loop fall
            // outside the grown lower slices (thus where the distance between
            // the loop centerline and original lower slices is >= half nozzle diameter
            extrusion_paths_append(paths,
                                   clip_extrusion(extrusion_path, lower_slices_paths, ClipperLib_Z::ctDifference),
                                   role_overhang, params.overhang_flow);
            ...
            if (!paths.empty()) {
                ...
                if (!extrusion.is_closed) {
                    ...
                    for (const ExtrusionPath &path : paths) {
                        ++point_occurrence[path.as_polyline().to_polyline().first_point()].occurrence;
                        ++point_occurrence[path.as_polyline().to_polyline().last_point()].occurrence;
                        if (path.role().is_bridge()) {
                            point_occurrence[path.as_polyline().to_polyline().first_point()].is_overhang = true;
                            point_occurrence[path.as_polyline().to_polyline().last_point()].is_overhang = true;
                        }
                    }
...
#ifdef ARACHNE_DEBUG
static void export_perimeters_to_svg(const std::string &path,
                                     const Polygons &contours,
                                     const Arachne::Perimeters &perimeters,
...
bool paths_touch(const ExtrusionPath &path_one, const ExtrusionPath &path_two, double limit_distance) {
    AABBTreeLines::LinesDistancer<Line> lines_two{path_two.as_polyline().to_polyline().lines()};
    for (size_t pt_idx = 0; pt_idx < path_one.polyline.size(); pt_idx++) {
        if (lines_two.distance_from_lines<false>(path_one.as_polyline().to_polyline().points[pt_idx]) <
            limit_distance) {
            return true;
        }
    }
    AABBTreeLines::LinesDistancer<Line> lines_one{path_one.as_polyline().to_polyline().lines()};
    for (size_t pt_idx = 0; pt_idx < path_two.polyline.size(); pt_idx++) {
        if (lines_one.distance_from_lines<false>(path_two.as_polyline().to_polyline().points[pt_idx]) <
            limit_distance) {
            return true;
        }
    }
...
Polylines reconnect_polylines(const Polylines &polylines, double limit_distance) {
    ...
    for (size_t a = 0; a < polylines.size(); a++) {
        ...
        for (size_t b = a + 1; b < polylines.size(); b++) {
            ...
            if ((base.last_point() - next.first_point()).cast<double>().squaredNorm() <
                limit_distance * limit_distance) {
                base.append(std::move(next));
                connected.erase(b);
            } else if ((base.last_point() - next.last_point()).cast<double>().squaredNorm() <
                       limit_distance * limit_distance) {
                base.points.insert(base.points.end(), next.points.rbegin(), next.points.rend());
                connected.erase(b);
            } else if ((base.first_point() - next.last_point()).cast<double>().squaredNorm() <
                       limit_distance * limit_distance) {
                next.append(std::move(base));
                base = std::move(next);
                base.reverse();
                connected.erase(b);
            } else if ((base.first_point() - next.first_point()).cast<double>().squaredNorm() <
                       limit_distance * limit_distance) {
                base.reverse();
                base.append(std::move(next));
                base.reverse();
...
ExtrusionPaths sort_extra_perimeters(const ExtrusionPaths &extra_perims,
    ...
    while (true) {
        if (next_idx == null_idx) { // find next pidx to print
            ...
            // we have valid next_idx, add it to the sorted paths, update dependencies, update current point and
            // potentialy set new next_idx
    ...
    for (const ExtrusionPath &path : sorted_paths) {
        if (!reconnected.empty() &&
            ...
            reconnected.back()
                .as_polyline()
                .to_polyline()
                .points.insert(reconnected.back().as_polyline().to_polyline().points.end(),
                               path.as_polyline().to_polyline().points.begin(),
                               path.as_polyline().to_polyline().points.end());
        } else {
...
// Function will generate extra perimeters clipped over nonbridgeable areas of the provided surface and returns both
// the new perimeters and Polygons filled by those clipped perimeters
std::tuple<std::vector<ExtrusionPaths>, Polygons> generate_extra_perimeters_over_overhangs(
    ...
    Polygons inset_anchors = diff(anchors,
                                  expand(overhangs, anchors_size + 0.1 * overhang_flow.scaled_width(),
    ...
    for (const ExPolygon &overhang : union_ex(to_expolygons(inset_overhang_area))) {
...
#ifdef EXTRA_PERIM_DEBUG_FILES
        {
            ...
            for (const Line &line : to_lines(
                     diff_pl(to_polylines(diff(real_overhang, anchors)), expand(anchors, float(SCALED_EPSILON)))))
        ...
        if (unbridgeable_area < 0.2 * area(real_overhang) && unsupp_dist < total_length(real_overhang) * 0.2) {
            ...
            while (continuation_loops >= 0) {
                ...
                // do not add the perimeter to result yet, first check if perimeter_polygon is not empty after shrinking
                // - this would mean
            ...
            perimeter_polygon = union_(perimeter_polygon, anchoring);
            inset_overhang_area_left_unfilled.insert(inset_overhang_area_left_unfilled.end(),
                                                     perimeter_polygon.begin(), perimeter_polygon.end());
            ...
            if (!overhang_region.empty()) {
                // there is a special case, where the first (or last) generated overhang perimeter eats all anchor
                // space. When this happens, the first overhang perimeter is also a closed loop, and needs special
                // check instead of the following simple is_anchored lambda, which checks only the first and last
                // point (not very useful on closed polyline)
                bool first_overhang_is_closed_and_anchored =
                    (overhang_region.front().first_point() == overhang_region.front().last_point() &&
                     !intersection_pl(overhang_region.front().as_polyline().to_polyline(), optimized_lower_slices)
                          .empty());
                ...
                if (!first_overhang_is_closed_and_anchored) {
                    ...
                    for (size_t i = 0; i < overhang_region.front().polyline.size(); i++) {
                        Point p = overhang_region.front().as_polyline().to_polyline()[i];
                        if (double d = lower_layer_aabb_tree.distance_from_lines<true>(p) < min_dist) {
                            min_dist = d;
                            min_dist_idx = i;
                        }
                    }
                    std::rotate(overhang_region.front().as_polyline().to_polyline().begin(),
                                overhang_region.front().as_polyline().to_polyline().begin() + min_dist_idx,
                                overhang_region.front().as_polyline().to_polyline().end());
                }
...
// Thanks, Cura developers, for implementing an algorithm for generating perimeters with variable width (Arachne) that
// is based on the paper "A framework for adaptive width control of dense contour-parallel toolpaths in fused
// deposition modeling"
void PerimeterGenerator::process_arachne(
    ...
    if (params.config.overhangs_width && lower_slices != nullptr && lower_slices_polygons_cache.empty()) {
    ...
    if (loop_number > 0 && ((params.config.only_one_perimeter_first_layer && params.layer_id == 0)))
        loop_number = 0;

    const int inner_loop_number = (params.config.only_one_perimeter_top && upper_slices != nullptr) ? loop_number - 1 : -1;

    ExPolygons last = offset_ex(surface.expolygon.simplify_p(params.scaled_resolution),
                                -float(ext_perimeter_width / 2. - ext_perimeter_spacing / 2.));
    Polygons last_p = to_polygons(last);
    Arachne::WallToolPaths wall_tool_paths(last_p, ext_perimeter_spacing, ext_perimeter_width, perimeter_spacing, perimeter_width, coord_t(loop_number + 1), coord_t(0), params.layer_height, params.config, params.print_config);
    ...
    if (inner_loop_number >= 0) {
        ...
        if (!top_expolygons.empty()) {
            ...
            // Get the not-top ExPolygons (including bridges) from current slices and expanded real top ExPolygons
            // (without bridges).
            ...
            Arachne::WallToolPaths inner_wall_tool_paths(not_top_polygons, ext_perimeter_spacing, ext_perimeter_width,
                                                         perimeter_spacing, perimeter_width,
            ...
            Arachne::WallToolPaths no_single_perimeter_tool_paths(last_p, ext_perimeter_spacing, ext_perimeter_width,
                                                                  perimeter_spacing, perimeter_width,
                                                                  coord_t(inner_loop_number + 2), 0,
                                                                  params.layer_height, params.config,
                                                                  params.print_config);
    ...
    if (lower_slices != nullptr && params.config.overhangs_width && params.config.extra_perimeters_on_overhangs &&
        ...
        if (!extra_perimeters.empty()) {
            ...
            old_entities.swap(this_islands_perimeters.m_entities);
            for (ExtrusionPaths &paths : extra_perimeters)
                this_islands_perimeters.append(std::move(paths));
            append(this_islands_perimeters.m_entities, old_entities);
...
void PerimeterGenerator::process_classic(
        ...
        for (int i = 0;; ++i) { // outer loop is 0
            ...
            if (i == 0) {
                ...
                if (has_gap_fill)
                    ...
                    append(gaps,
                           diff_ex(offset(last, -float(0.5 * distance)),
            ...
            if (i == 0 && i != loop_number && upper_slices != nullptr) {
                ...
                if (lower_slices != nullptr) {
                    const float bridge_offset = 1.5f *
                        float(std::max<coord_t>(ext_perimeter_spacing, perimeter_width));
                    const Polygons lower_slices_clipped =
                ...
                const Polygons upper_slices_clipped =
                    expand(ClipperUtils::clip_clipper_polygons_with_subject_bbox(*upper_slices,
                ...
                if (!top_polygons.empty()) {
                    ...
                    // Get the not-top ExPolygons (including bridges) from current slices and expanded real top
                    // ExPolygons (without bridges).
                    const ExPolygons not_top_polygons = diff_ex(last,
                                                                offset_ex(top_polygons,
                                                                          float(top_surface_offset) +
                                                                              top_surface_min_width -
                                                                              (float(ext_perimeter_spacing) / 2.f)),
                                                                ApplySafetyOffset::Yes);

                    // Get difference between top ExPolygons without bridges and the area defined by the virtual
                    // second perimeter.
                    const ExPolygons top_gap = diff_ex(top_polygons, fill_clip);

                    // Get top infill surface ExPolygons (without bridges) using the difference between the area
                    // defined by the virtual second perimeter and non-top ExPolygons.
                    top_fills = diff_ex(fill_clip, not_top_polygons, ApplySafetyOffset::Yes);

                    // Set the clip to the external perimeter but go back inside by infill_extrusion_width/2 to ensure
                    // the extrusion won't go outside even with a 100% overlap.
                    fill_clip = offset_ex(last,
                                          float((coordf_t(ext_perimeter_spacing) / 2.) -
    ...
    if (!gaps.empty()) {
        ...
        if (!polylines.empty()) {
            ...
            variable_width_classic(polylines, ExtrusionRole::GapFill, params.solid_infill_flow, gap_fill.m_entities);
            ...
            out_gap_fill.append(std::move(gap_fill.m_entities));
        }
    }
