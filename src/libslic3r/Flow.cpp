///|/ Copyright (c) Prusa Research 2016 - 2023 Pavel Mikuš @Godrak, Oleksandra Iushchenko @YuSanka, Vojtěch Bubník
///@bubnikv, Tomáš Mészáros @tamasmeszaros
...
FlowErrorNegativeSpacing::FlowErrorNegativeSpacing()
    : FlowError("Flow::spacing() produced negative spacing. Did you set some extrusion width too small, or the "
                "(maximum) layer height too high?") {}

FlowErrorNegativeFlow::FlowErrorNegativeFlow()
    : FlowError("Flow::mm3_per_mm() produced negative flow. Did you set some extrusion width too small, or the "
                "(maximum) layer height too high?") {}
...
float Flow::auto_extrusion_width(FlowRole role, float nozzle_diameter) {
    ...
    case frExternalPerimeter: return nozzle_diameter;
    default:
    case frPerimeter:
    case frSolidInfill:
...
// Used to provide hints to the user on default extrusion width values, and to provide reasonable values to the
// PlaceholderParser.
...
const ConfigOptionFloatOrPercent *Flow::extrusion_width_option(std::string opt_key,
                                                               const ConfigOptionResolver &config) {
    ...
    if (opt && boost::starts_with(opt_key, "first_layer_")) {
        const ConfigOptionFloatOrPercent *opt_test = config.option<ConfigOptionFloatOrPercent>(
    ...
    if (opt->value == 0. && boost::starts_with(opt_key, "skirt")) {
        ...
        const ConfigOptionFloatOrPercent *opt_first_layer_extrusion_width = config.option<ConfigOptionFloatOrPercent>(
            "first_layer_extrusion_width");
        const ConfigOptionInt *opt_skirt_height = config.option<ConfigOptionInt>("skirt_height");
        const ConfigOptionEnum<DraftShield> *opt_draft_shield = config.option<ConfigOptionEnum<DraftShield>>(
            "draft_shield");
        ...
        if (opt_first_layer_extrusion_width && opt_first_layer_extrusion_width->is_enabled() &&
            opt_first_layer_extrusion_width->value > 0 && opt_draft_shield && opt_skirt_height &&
            opt_skirt_height->value == 1 && opt_draft_shield->value != DraftShield::dsDisabled)
...
// used to get brim & skirt extrusion config
const ConfigOptionFloatOrPercent *Flow::extrusion_spacing_option(std::string opt_key,
                                                                 const ConfigOptionResolver &config) {
    ...
    if (!opt && boost::starts_with(opt_key, "brim_")) {
        const ConfigOptionFloatOrPercent *optTest = config.option<ConfigOptionFloatOrPercent>(
    ...
    if (opt->value == 0. && boost::starts_with(opt_key, "skirt")) {
        ...
        const ConfigOptionFloatOrPercent *opt_first_layer_extrusion_spacing =
            config.option<ConfigOptionFloatOrPercent>("first_layer_extrusion_spacing");
        const ConfigOptionInt *opt_skirt_height = config.option<ConfigOptionInt>("skirt_height");
        const ConfigOptionEnum<DraftShield> *opt_draft_shield = config.option<ConfigOptionEnum<DraftShield>>(
            "draft_shield");
        ...
        if (opt_first_layer_extrusion_spacing && opt_draft_shield && opt_skirt_height &&
            opt_first_layer_extrusion_spacing->value > 0 && opt_skirt_height->value == 1 &&
            opt_draft_shield->value != DraftShield::dsDisabled)
...
Flow Flow::new_from_config(FlowRole role,
                           const DynamicConfig &print_config,
                           float nozzle_diameter,
                           float layer_height,
                           float filament_max_overlap,
                           bool first_layer) {
    ...
    if (role == frExternalPerimeter) {
        ...
        if (!config_spacing.is_phony() && config_spacing.value != 0) {
            ...
            config_spacing.value = rounded_rectangle_extrusion_spacing(
                rounded_rectangle_extrusion_width_from_spacing(raw_spacing, layer_height, 0.5f), layer_height, 1.f);
        }
    ...
    if (first_layer) {
        ...
        if ((role == frInfill || role == frSolidInfill || role == frTopSolidInfill) && opt_fli_width && opt_fli_width->is_enabled()) {
            config_width.set(*opt_fli_width);
            config_spacing.set(*print_config.option("first_layer_infill_extrusion_spacing"));
        } else if (opt_fl_width && opt_fl_width->is_enabled()) {
    ...
    // Here this->extruder(role) - 1 may underflow to MAX_INT, but then the get_at() will follback to zero'th element,
...
Flow Flow::new_from_config_width(FlowRole role,
                                 const ConfigOptionFloatOrPercent &width,
                                 const ConfigOptionFloatOrPercent &spacing,
                                 float nozzle_diameter,
                                 float height,
                                 float spacing_ratio,
    ...
    if (bridge_flow_ratio > 0) {
        ...
        if (!width.is_phony()) {
            ...
            if (!spacing.percent && spacing.value == 0.) {
                ...
                // If user set a manual value, use it.
                return new_from_spacing(float(spacing.get_abs_value(nozzle_diameter)), nozzle_diameter, height,
                                        spacing_ratio, false);
            }
        }
    }

    return Flow(w, height, rounded_rectangle_extrusion_spacing(w, height, spacing_ratio), nozzle_diameter,
                spacing_ratio, bridge_flow_ratio > 0);
}
...
Flow Flow::new_from_spacing(float spacing, float nozzle_diameter, float height, float spacing_ratio, bool bridge) {
    ...
    // For normal extrusons, extrusion width is wider than the spacing due to the rounding and squishing of the
    // extrusions. For bridge extrusions, the extrusions are placed with a tiny BRIDGE_EXTRA_SPACING gaps between the
    // threads.
    float width = float(bridge ? (spacing /*- BRIDGE_EXTRA_SPACING_MULT (0.125) * nozzle_diameter*/) :
...
Flow Flow::new_from_width(float width, float nozzle_diameter, float height, float spacing_ratio, bool bridge) {
    ...
    float spacing = float(bridge ? (width /*- BRIDGE_EXTRA_SPACING_MULT (0.125) * nozzle_diameter*/) :
...
float Flow::spacing() const {
    ...
    // rectangle with semicircles at the end
...
#else
    float res = float(this->bridge() ? (this->width() /*+ BRIDGE_EXTRA_SPACING_MULT * nozzle_diameter*/) :
                                       (this->width() - this->height() * (1. - 0.25 * PI) * m_spacing_ratio));
#endif
    //    assert(res > 0.f);
    if (res <= 0.f)
...
// Adjust the width / height of a rounded extrusion model to reach the prescribed cross section area while maintaining
// extrusion spacing.
Flow Flow::with_cross_section(float area_new) const {
    ...
    if (area_new > area + EPSILON) {
        ...
        if (new_full_spacing > m_spacing) {
            ...
            return Flow(rounded_rectangle_extrusion_width_from_spacing(m_spacing, height, m_spacing_ratio), height,
                        m_spacing, m_nozzle_diameter, m_spacing_ratio, false);
        } else {
            return this->with_width(
                rounded_rectangle_extrusion_width_from_spacing(area / m_height, m_height, m_spacing_ratio));
        }
...
float Flow::spacing(const Flow &other) const {
    ...
    float res = float(this->bridge() || other.bridge() ? 0.5 * this->width() + 0.5 * other.width() :
...
Flow support_material_flow(const PrintObject *object, float layer_height) {
    ...
    const ConfigOptionFloatOrPercent &width = (object->config().support_material_extrusion_width.value > 0) ?
        object->config().support_material_extrusion_width :
        object->config().extrusion_width;
    const ConfigOptionFloatOrPercent &spacing = (object->config().support_material_extrusion_width.value > 0) ?
        object->config().support_material_extrusion_width :
        object->config().extrusion_spacing;
    ...
    return Flow::new_from_config_width(
        frSupportMaterial,
        // The width parameter accepted by new_from_config_width is of type ConfigOptionFloatOrPercent, the Flow class
        // takes care of the percent to value substitution.
        width, spacing,
        // if object->config().support_material_extruder == 0 (which means to not trigger tool change, but use the
        // current extruder instead), get_at will return the 0th component.
        float(nzd), layer_height,
        extruder_id < 0 ?
            1 :
            object->config().get_computed_value("filament_max_overlap",
                                                extruder_id), // if can get an extruder, then use its param, or use
                                                              // full overlap if we don't know the extruder id.
        // bridge_flow_ratio
        0.f);
}
...
Flow support_material_1st_layer_flow(const PrintObject *object, float layer_height) {
    ...
    return Flow::new_from_config_width(
        ...
        (spacing.value > 0) ?
            spacing :
            object->config().extrusion_spacing, // can be used if first_layer_extrusion_width is phony
        float(print_config.nozzle_diameter.get_at(extruder_id)), slice_height,
        extruder_id < 0 ?
            1 :
            object->config().get_computed_value("filament_max_overlap",
                                                extruder_id), // if can get an extruder, then use its param, or use
...
Flow support_material_interface_flow(const PrintObject *object, float layer_height) {
    ...
    return Flow::new_from_config_width(
        ...
        // takes care of the percent to value substitution.
        width, spacing,
        // if object->config().support_material_interface_extruder == 0 (which means to not trigger tool change, but
