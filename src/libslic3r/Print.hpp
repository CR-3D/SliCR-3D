///|/ Copyright (c) Prusa Research 2016 - 2023 Lukáš Matěna @lukasmatena, Enrico Turri @enricoturri1966, Vojtěch
///Bubník @bubnikv, Tomáš Mészáros @tamasmeszaros, Pavel Mikuš @Godrak, Lukáš Hejl @hejllukas, Filip Sykala @Jony01,
///Oleksandra Iushchenko @YuSanka, Vojtěch Král @vojtechkral
...
enum PrintObjectStep : uint8_t {
    ...
    posSimplifyPath, // simplify &  arc fitting from BB
    posCount,
};
...
public:
    ...
    void config_apply_only(const ConfigBase &other,
...
public:
    ...
    struct LayerRangeRegions
    {
        ...
        // Sorted in the order of their source ModelVolumes, thus reflecting the order of region clipping, modifier
        // overrides etc.
    ...
    // Transformation of this ModelObject into one of the associated PrintObjects (all PrintObjects derived from a
    // single modelObject differ by a Z rotation only). This transformation is used to calculate VolumeExtents.
...
public:
    ...
    // Height is used for slicing, for sorting the objects by height for sequential printing and for checking vertical
    // clearence in sequential print mode. The height is snug.
    ...
    SupportLayerPtrs::iterator insert_support_layer(SupportLayerPtrs::const_iterator pos,
                                                    size_t id,
                                                    size_t interface_id,
                                                    coordf_t height,
                                                    coordf_t print_z,
    ...
    // Initialize the layer_height_profile from the model_object's layer_height_profile, from model_object's layer
    // height table, or from slicing parameters. Returns true, if the layer_height_profile was changed.
    ...
    static std::shared_ptr<SlicingParameters> slicing_parameters(const DynamicPrintConfig &full_config,
...
protected:
    ...
    // Called on main thread with stopped or paused background processing to let PrintObject release data for its
    // milestones that were invalidated or canceled.
    void cleanup();
...
private:
    ...
    ExPolygons _shrink_contour_holes(double contour_delta,
                                     double default_delta,
                                     double convex_delta,
                                     const ExPolygons &input) const;
    ...
    // pair < adaptive , support>, filled by prepare_adaptive_infill_data() (in bridge_over_infill() in
    // prepare_infill()) and used in infill()
...
struct ConflictResult
{
    ...
    int layer = -1;
    ConflictResult(
        const std::string &objName1, const std::string &objName2, double height, const void *obj1, const void *obj2)
        : _objName1(objName1), _objName2(objName2), _height(height), _obj1(obj1), _obj2(obj2) {}
    ConflictResult(const std::string &objName1,
                   const std::string &objName2,
                   double height,
                   const void *obj1,
                   const void *obj2,
                   int layer_id)
...
public:
    ...
    // If preview_data is not null, the preview_data is filled in for the G-code visualization (not used by the
    // command line Slic3r).
    ...
    // Return flow parameters for the skirt printed by the specified extruder.
    ...
    const PrintObject *get_print_object_by_model_object_id(ObjectID object_id) const {
        auto it = std::find_if(m_objects.begin(), m_objects.end(), [object_id](const PrintObject *obj) {
    ...
    // in public to invalidate gcode when the physical printer change. It's needed if we allow the gcode macro to read
    // these values.
    bool invalidate_step(PrintStep step);

    // just a little wrapper to let the user know that this print can only be modified to emit warnings & update
    // advancement status, change stats.
    // TODO: have the status out of the printbase class and into another one, so we can have a const print & a mutable
    // statusmonitor
    class StatusMonitor
    {
    private:
    ...
    public:
        ...
        // need this extra method because active_step_add_warning is protected and so need the friend status, and
        // Gcode has it.
