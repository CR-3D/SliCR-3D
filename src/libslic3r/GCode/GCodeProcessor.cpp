...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    ...
    { EProducer::SliCR3D,     "generated by SliCR-3D"},
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        try {
        if (m_producer == EProducer::SliCR3D || m_producer == EProducer::PrusaSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r || m_producer == EProducer::SuperSlicer) {
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled 
        || m_producer == EProducer::SliCR3D 
        || m_producer == EProducer::PrusaSlicer 
        || m_producer == EProducer::Slic3r 
        || m_producer == EProducer::SuperSlicer) {
        // height tag
