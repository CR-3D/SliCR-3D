...
protected:
    ...
    void set_task_impl(const TaskParams &params, std::vector<PrintObject *> &print_objects) {
        ...
        if (params.single_model_object.valid()) {
            ...
            // Now the background process is either stopped, or it is inside one of the print object steps to be
            // calculated anyway.
            ...
            for (PrintObject *print_object : print_objects)
                for (int istep = 0; istep < n_object_steps; ++istep) {
                    ...
                    if (print_object->is_step_started_unguarded(PrintObjectStepEnum(istep))) {
                        // This step is running, and the state cannot be changed due to the this->state_mutex() being
                        // locked. It is safe to manipulate m_stepmask of other PrintObjects and Print now.
                        running = true;
                        goto loop_end;
                    }
                }
