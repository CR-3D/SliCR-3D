...
void InterlockingGenerator::generate_interlocking_structure(PrintObject &print_object)
{
    ...
    for (size_t region_a_index = 0; region_a_index < print_object.num_printing_regions(); region_a_index++) {
        ...
        const auto         extruder_nr_a = region_a.extruder(FlowRole::frExternalPerimeter, print_object);
        ...
        for (size_t region_b_index = region_a_index + 1; region_b_index < print_object.num_printing_regions(); region_b_index++) {
            ...
            const auto         extruder_nr_b = region_b.extruder(FlowRole::frExternalPerimeter, print_object);
...
std::pair<ExPolygons, ExPolygons> InterlockingGenerator::growBorderAreasPerpendicular(const ExPolygons& a, const ExPolygons& b, const coord_t& detect) const
{
    const coord_t min_line =
        std::min(print_object.printing_region(region_a_index).flow(print_object, frExternalPerimeter, 0.1, 2).scaled_width(),
                 print_object.printing_region(region_b_index).flow(print_object, frExternalPerimeter, 0.1, 2).scaled_width());
...
void InterlockingGenerator::handleThinAreas(const std::unordered_set<GridPoint3>& has_all_meshes) const
{
    ...
    const coord_t close_gaps =
        std::min(print_object.printing_region(region_a_index).flow(print_object, frExternalPerimeter, 0.1, 2).scaled_width(),
                 print_object.printing_region(region_b_index).flow(print_object, frExternalPerimeter, 0.1, 2).scaled_width()) / 4;
    ...
    for (size_t layer_nr = 0; layer_nr < print_object.layer_count(); layer_nr++){
        ...
        layer->get_region(region_a_index)->m_slices.set(closing_ex(diff_ex(union_ex(polys_a, thin_expansion_a), thin_expansion_b), close_gaps), stPosInternal);
        layer->get_region(region_b_index)->m_slices.set(closing_ex(diff_ex(union_ex(polys_b, thin_expansion_b), thin_expansion_a), close_gaps), stPosInternal);
    }
}
