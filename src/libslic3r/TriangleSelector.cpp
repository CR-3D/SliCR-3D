...
std::vector<int> TriangleSelector::get_all_touching_triangles(int facet_idx, const Vec3i32 &neighbors, const Vec3i32 &neighbors_propagated) const {
    ...
    const Vec3i32 vertices = { m_triangles[facet_idx].verts_idxs[0], m_triangles[facet_idx].verts_idxs[1], m_triangles[facet_idx].verts_idxs[2] };
...
void TriangleSelector::bucket_fill_fill_gaps(const std::vector<int> &gap_fill_candidate_facets, const float bucket_fill_gap_area,
                                             const TriangleStateType start_facet_state, const std::vector<Vec3i32> &neighbors,
                                             const std::vector<Vec3i32> &neighbors_propagated) {
    std::vector<bool> visited(m_triangles.size(), false);
...
bool TriangleSelector::select_triangle_recursive(int facet_idx, const Vec3i32 &neighbors, TriangleStateType type, bool triangle_splitting) {
    assert(facet_idx < int(m_triangles.size()));
...
void TriangleSelector::perform_split(int facet_idx, const Vec3i32 &neighbors, TriangleStateType old_state) {
...
template<AdditionalMeshInfo facet_info>
void TriangleSelector::get_facets_split_by_tjoints(const Vec3i32 &vertices, const Vec3i32 &neighbors, const uint8_t color, std::vector<stl_triangle_vertex_indices> &out_triangles, std::vector<uint8_t> &out_colors) const {
