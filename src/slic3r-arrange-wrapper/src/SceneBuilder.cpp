...
coord_t get_skirt_inset(const Print &fffprint)
{
    ...
    if (fffprint.has_skirt()) {
        ...
        ? 0
        : fffprint.skirt_flow(0).width();
        skirt_inset = fffprint.config().skirts.value * skirtflow
        + fffprint.config().skirt_distance.value;
    }
    
    return scaled<coord_t>(skirt_inset);
}

coord_t brim_offset(const PrintObject &po)
{
    const float    brim_separation = po.config().brim_separation.get_float();
    const float    brim_width      = po.config().brim_width.get_float();
    const bool     has_outer_brim  = BrimType::btOuterOnly;

    // How wide is the brim? (in scaled units)
    return has_outer_brim ? scaled<coord_t>(brim_width + brim_separation) : 0;
}
...
{
    Vec2i32 crd(m_xstrider.get_bed_index(obj),
               m_ystrider.get_bed_index(obj));
    return BedsGrid::grid_coords2index(crd.cast<coord_t>());
}
...
bool GridStriderVBedHandler::assign_bed(VBedPlaceable &inst, int bed_idx)
{
    ...
    Vec2i32 crd = BedsGrid::index2grid_coords(bed_idx).cast<int>();
